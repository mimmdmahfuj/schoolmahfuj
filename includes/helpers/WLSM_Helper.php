...
class WLSM_Helper {
	...
	/**
	 * Assigns school management capability to specific users
	 *
	 * @param int|array $user_ids User ID or array of user IDs
	 * @return void
	 */
	public static function assign_school_management_capability($user_ids) {
		...
		foreach ($user_ids as $user_id) {
			...
			if ($user) {
				$user->add_cap('manage_school_management');
			}
		}
	}
	...
	public static function send_firebase_notification($token, $title, $body, $school_id)
	{
		// Load Firebase SDK
		...
		$firebase_settings = $wpdb->get_row($wpdb->prepare(
			'SELECT setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "firebase"',
			$school_id
		));

		if (!$firebase_settings) {
			error_log('Firebase settings not found.');
			return false;
		}
		$settings = unserialize($firebase_settings->setting_value);
		$json_file_path = $settings['json_file_path'] ?? '';
		if (empty($json_file_path) || !file_exists($json_file_path)) {
			error_log('Firebase JSON file not found.');
			return false;
		}
		try {
			...
			error_log('Firebase Messaging Error: ' . $e->getMessage());
			return false;
		} catch (\Exception $e) {
	...
	public static function is_json_file($file)
	{
		// First, check if the file has been uploaded and the array structure is correct
		...
		if ($file_extension !== 'json') {
			return false; // Not a JSON file based on extension
		}

		// Check the MIME type of the file using the fileinfo extension
		if (extension_loaded('fileinfo')) {
			...
			if ($mime_type !== 'application/json') {
				return false; // Not a JSON file based on MIME type
			}
		} else {
			// Fallback to browser-reported MIME type
			if ($file['type'] !== 'application/json') {
				return false; // Not a JSON file based on browser-reported MIME type
			}
		}
		...
		// Finally, check if the file content is valid JSON
	...
	public static function gender_list( $school_id = null ) {
		// Default gender list
		$default_list = array(
		...
		// If no school_id provided, return default
		if ( ! $school_id ) {
			return $default_list;
		}

		// Get school-specific gender list from settings
		$settings_registration = WLSM_M_Setting::get_settings_registration( $school_id );

		if ( ! empty( $settings_registration['gender_list'] ) && is_array( $settings_registration['gender_list'] ) ) {
			// Ensure Male and Female are always present
			$custom_list = $settings_registration['gender_list'];
			if ( ! isset( $custom_list['male'] ) ) {
				$custom_list['male'] = esc_html__( 'Male', 'school-management' );
			}
			if ( ! isset( $custom_list['female'] ) ) {
				$custom_list['female'] = esc_html__( 'Female', 'school-management' );
			}
			return $custom_list;
		}
	...
	public static function category_list() {
		return array(
			'st'   => esc_html__( 'ST', 'school-management' ),
			'sc' => esc_html__( 'SC', 'school-management' ),
			'obc'  => esc_html__( 'OBC', 'school-management' ),
			'gen'  => esc_html__( 'GEN', 'school-management' ),
		);
	}
	...
	public static function certificate_field_labels() {
		return array(
			...
			'pass_out_year'       => esc_html__( 'Passout Year', 'school-management' ),
			'school_class'        => esc_html__( 'Previous School Class', 'school-management' ),
			'school_address'      => esc_html__( 'Previous School Address', 'school-management' ),
			'school_name'         => esc_html__( 'Previous School Name', 'school-management' ),
			'birth_place'         => esc_html__( 'Birth Place', 'school-management'),
			'mother_tongue'       => esc_html__( 'Mother Tongue', 'school-management'),
			'dob_in_words'        => esc_html__( 'Date of Birth in Words', 'school-management'),
			'religion'            => esc_html__( 'Religion', 'school-management'),
		);
	}
	...
	public static function charts() {
		return array(
			...
			'monthly_income_expense' => esc_html__( 'Monthly Donation / Expense', 'school-management' ),
			'accounting_payment_expense' => esc_html__( 'Payments vs Expenses', 'school-management' )
		);
	}
	...
	public static function default_chart_types() {
		return array(
			...
			'accounting_payment_expense' => 'bar'
		);
	}
	...
	public static function getAssignedSubjects($adminId ) {
		global $wpdb;
		$query = $wpdb->prepare("
        SELECT s.ID, s.label as subject_name , s.code, s.type
    ...
    ", $adminId);
		...
		// Add an empty subject at the beginning of the array
		array_unshift($results, (object) ['ID' => '', 'subject_name' => 'Select subject', 'code' => '', 'type' => '']);
