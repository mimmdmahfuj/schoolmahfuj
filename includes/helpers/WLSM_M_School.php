...
class WLSM_M_School {
	...
	public static function fetch_school( $id ) {
		global $wpdb;
		$school = $wpdb->get_row( $wpdb->prepare( 'SELECT s.ID, s.label, s.phone, s.email, s.address, s.description, s.registration_number, s.is_active, s.enrollment_prefix, s.enrollment_base, s.enrollment_padding, s.admission_prefix, s.admission_base, s.admission_padding, s.category_id FROM ' . WLSM_SCHOOLS . ' as s WHERE s.ID = %d', $id ) );
		return $school;
	}
	...
	public static function get_description_text( $description ) {
		if ( $description ) {
			return $description;
		}
		return '-';
