...
class WLSM_M_Staff_Accountant {
	...
	public static function 	get_invoices_report_total( $school_id, $session_id, $class_id, $section_id, $payment_method ) {
		global $wpdb;
		...
		if ($section_id === 0) {
			...
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			LEFT OUTER JOIN ' . WLSM_PAYMENTS . ' as p ON p.invoice_id = i.ID
			WHERE cs.school_id = %d AND ss.ID = %d AND c.ID = %d AND p.payment_method =%s', $school_id , $session_id , $class_id, $payment_method  ) );
		} else {
			$query = $wpdb->get_row( $wpdb->prepare('SELECT COALESCE(SUM(p.amount), 0) as paid, SUM(i.amount - COALESCE(p.amount, 0)) as due FROM ' . WLSM_INVOICES . ' as i
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = i.student_record_id
			JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
			...
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			LEFT OUTER JOIN ' . WLSM_PAYMENTS . ' as p ON p.invoice_id = i.ID
			WHERE cs.school_id = %d AND ss.ID = %d AND c.ID = %d AND se.ID = %d AND p.payment_method =%s', $school_id , $session_id , $class_id , $section_id, $payment_method ) );
		}
		return $query;
	}
	...
	public static function fetch_expense( $school_id, $id ) {
		global $wpdb;
		$expense = $wpdb->get_row( $wpdb->prepare( 'SELECT ep.ID, ep.label, ep.invoice_number, ep.amount, ep.expense_date, ep.note, ep.attachment, ep.expense_category_id, ep.supplier_name, ep.receiver_signature FROM ' . WLSM_EXPENSES . ' as ep
		WHERE ep.school_id = %d AND ep.ID = %d', $school_id, $id ) );
		return $expense;
	}
	...
	public static function fetch_income( $school_id, $id ) {
		global $wpdb;
		$income = $wpdb->get_row( $wpdb->prepare( 'SELECT im.ID, im.label, im.invoice_number, im.amount, im.income_date, im.note, im.attachment, im.income_category_id, im.doner_name, im.receiver_signature FROM ' . WLSM_INCOME . ' as im
		WHERE im.school_id = %d AND im.ID = %d', $school_id, $id ) );
		return $income;
	}
	...
	public static function fetch_fee_query( $school_id, $restrict_to_section = null )
	{
		...
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.class_id = ft.class_id
        JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = cs.default_section_id
		JOIN ' . WLSM_CLASSES . ' as c ON c.ID = ft.class_id
		WHERE ft.school_id = ' . absint( $school_id ) . $section_where;
		return $query;
	}
	...
	public static function fetch_fee_query_count( $school_id, $restrict_to_section = null )
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND cs.default_section_id = ' . absint( $restrict_to_section );
		} else {
	...
	public static function fetch_fee( $school_id, $id ) {
		global $wpdb;
		$fee = $wpdb->get_row( $wpdb->prepare( 'SELECT ft.ID, ft.label, ft.amount, ft.period, ft.active_on_admission, ft.active_on_dashboard,  ft.class_id, ft.student_type FROM ' . WLSM_FEES . ' as ft
		WHERE ft.school_id = %d AND ft.ID = %d', $school_id, $id ) );
		return $fee;
	}
	...
	public static function fetch_concession_types_query( $school_id ) {
		$query = 'SELECT ct.ID, ct.concession_name, ct.concession_type, ct.percentage_value, ct.fixed_amount, ct.eligibility_criteria, ct.is_active, c.label as class_label FROM ' . WLSM_CONCESSION_TYPES . ' as ct
	...
	public static function fetch_concession_types_query_count( $school_id ) {
	...
	public static function fetch_concession_types( $school_id, $id ) {
		global $wpdb;
		$concession_types = $wpdb->get_row( $wpdb->prepare( 'SELECT ct.ID, ct.concession_name, ct.concession_type, ct.percentage_value, ct.fixed_amount, ct.eligibility_criteria, ct.is_active, ct.class_id FROM ' . WLSM_CONCESSION_TYPES . ' as ct
	...
	public static function get_concession_fee_mappings( $concession_id ) {
		...
		$mappings = $wpdb->get_results( $wpdb->prepare( 
			'SELECT cfm.fee_type_id, ft.label as fee_label FROM ' . WLSM_CONCESSION_FEE_MAPPINGS . ' as cfm
	...
	public static function calculate_concession_amount( $fee_amount, $concession_type, $percentage_value, $fixed_amount ) {
		if ( $concession_type === 'percentage' && $percentage_value > 0 ) {
			return ( $fee_amount * $percentage_value ) / 100;
		} else if ( $concession_type === 'fixed_amount' && $fixed_amount > 0 ) {
			return min( $fixed_amount, $fee_amount ); // Don't exceed fee amount
		}
		return 0;
	}
