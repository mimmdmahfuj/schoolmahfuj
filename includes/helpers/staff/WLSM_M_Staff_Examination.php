...
class WLSM_M_Staff_Examination {
	...
	public static function fetch_exam_papers( $school_id, $exam_id ) {
		global $wpdb;
		$papers = $wpdb->get_results( $wpdb->prepare( 'SELECT DISTINCT ep.ID, s.label as subject_label, s.type as subject_type, ep.paper_code, ep.paper_order, ep.paper_date, ep.start_time, ep.end_time, ep.room_number, ep.maximum_marks, ep.subject_id FROM ' . WLSM_EXAM_PAPERS . ' as ep
	...
	public static function fetch_exam_papers_student($school_id, $exam_id, $student_id) {
		global $wpdb;
		$papers = $wpdb->get_results($wpdb->prepare('SELECT ep.ID, s.label as subject_label, s.type as subject_type, ep.paper_code, ep.paper_order, ep.paper_date, ep.start_time, ep.end_time, ep.room_number, ep.maximum_marks, ep.exam_id FROM ' . WLSM_EXAM_PAPERS . ' AS ep
			JOIN ' . WLSM_EXAMS . ' AS ex ON ex.ID = ep.exam_id
			LEFT JOIN ' . WLSM_SUBJECTS . ' AS s ON s.ID = ep.subject_id
	...
	public static function get_class_school_exams_time_table($school_id, $class_school_id, $start_date = null, $end_date = null) {
		global $wpdb;

		$query = 'SELECT DISTINCT ex.ID, ex.label as exam_title, ex.start_date, ex.end_date
		...
		$params = [$school_id, $class_school_id];

		if ($start_date) {
			$query .= ' AND ex.start_date >= %s';
			$params[] = $start_date;
		}

		if ($end_date) {
			$query .= ' AND ex.end_date <= %s';
			$params[] = $end_date;
		}

		$query .= ' ORDER BY ex.start_date DESC, ex.ID DESC';
	...
	public static function get_class_school_exams_academic_report($school_id, $class_school_id, $academic_report_id) {
		...
		$exams = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT DISTINCT ex.ID, ex.label AS exam_title, ex.show_rank, ex.show_remark, ex.psychomotor_analysis, ex.psychomotor, ex.start_date, ex.end_date, ex.results_obtained_marks, ex.enable_total_marks, ex.teacher_signature, ex.grade_criteria
				FROM " . WLSM_EXAMS . " AS ex
				JOIN " . WLSM_CLASS_SCHOOL_EXAM . " AS csex ON csex.exam_id = ex.ID
				JOIN " . WLSM_SCHOOLS . " AS s ON s.ID = ex.school_id
	...
	public static function get_exam_papers_by_admit_card( $school_id, $admit_card_id ) {
		global $wpdb;
		$exam_papers = $wpdb->get_results( $wpdb->prepare( 'SELECT ep.ID, s.label as subject_label, s.type as subject_type, ep.paper_code, ep.maximum_marks, ep.subject_id FROM ' . WLSM_EXAM_PAPERS . ' as ep
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
			LEFT JOIN ' . WLSM_SUBJECTS . ' as s ON s.ID = ep.subject_id
	...
	public static function get_exam_papers_by_exam_id( $school_id, $exam_id ) {
		global $wpdb;
		$exam_papers = $wpdb->get_results( $wpdb->prepare( 'SELECT ep.ID, sub.label as subject_label, sub.type as subject_type, ep.paper_code, ep.maximum_marks, ep.subject_id FROM ' . WLSM_EXAM_PAPERS . ' as ep
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
			LEFT JOIN ' . WLSM_SUBJECTS . ' as sub ON sub.ID = ep.subject_id
			JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = ex.school_id
