...
class WLSM_M_Staff_Class
{
	...
	public static function fetch_classes_query($school_id, $session_id, $restrict_to_section = false)
	{
		...
		$query = 'SELECT c.ID, c.label,
						 COUNT(DISTINCT se.ID) as sections_count,
						 COUNT(DISTINCT sr.ID) as students_count,
						 SUM(CASE WHEN sr.gender = "male" THEN 1 ELSE 0 END) as male_students_count,
						 SUM(CASE WHEN sr.gender = "female" THEN 1 ELSE 0 END) as female_students_count
				  FROM ' . WLSM_CLASS_SCHOOL . ' as cs
				  ...
				  WHERE cs.school_id = ' . absint($school_id) . $section_where;
		return $query;
	}
	...
	public static function fetch_classes_query_count($school_id, $restrict_to_section = false)
	{
		if ( $restrict_to_section ) {
		...
		$query = 'SELECT COUNT(DISTINCT c.ID) FROM ' . WLSM_CLASS_SCHOOL . ' as cs JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id WHERE cs.school_id =' . absint($school_id) . $section_where;
		return $query;
	}
	...
	public static function get_class($school_id, $class_id) {
		global $wpdb;
		$class = $wpdb->get_row(
			$wpdb->prepare('SELECT cs.ID, cs.default_section_id FROM ' . WLSM_CLASS_SCHOOL . ' as cs
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			WHERE cs.class_id = %d AND cs.school_id = %d', $class_id, $school_id)
		);
		return $class;
	}
	...
	{
		$query = 'SELECT se.ID, se.label, cs.class_id as class_id, cs.default_section_id,
						 COUNT(sr.ID) as students_count,
						 SUM(CASE WHEN sr.gender = "male" THEN 1 ELSE 0 END) as male_students_count,
						 SUM(CASE WHEN sr.gender = "female" THEN 1 ELSE 0 END) as female_students_count
				  FROM ' . WLSM_SECTIONS . ' as se
				  JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
				  JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = cs.school_id
				  LEFT OUTER JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.section_id = se.ID AND sr.session_id = ' . absint($session_id) . '
	...
	public static function get_subject_type_by_label( $label) {
		global $wpdb;
		$subject_type = $wpdb->get_row($wpdb->prepare('SELECT st.ID, st.label FROM ' . WLSM_SUBJECT_TYPES . ' as st WHERE st.label = %s', $label));
		return $subject_type;
	}
	...
	public static function fetch_sections_by_class_id($school_id, $class_id)
	{
		global $wpdb;
		// Assuming 'class_id' is the common column in WLSM_CLASS_SCHOOL that references 'class_school_id' in WLSM_SECTIONS
		$sections = $wpdb->get_results($wpdb->prepare(
			'SELECT se.ID, se.label
			FROM ' . WLSM_SECTIONS . ' as se
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON se.class_school_id = cs.ID
			WHERE cs.school_id = %d AND cs.class_id = %d',
			$school_id, $class_id
		));
		return $sections;
	}

	public static function fetch_class_by_section_id($school_id, $section_id)
	{
		global $wpdb;
		$class = $wpdb->get_results($wpdb->prepare('SELECT DISTINCT(c.ID), c.label FROM ' . WLSM_CLASS_SCHOOL . ' as cs
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			WHERE cs.school_id = %d AND cs.default_section_id = %d ORDER BY c.ID ASC', $school_id, $section_id));
		return $class;
	}

	public static function fetch_students_by_section_id($section_id)
	{
		global $wpdb;
		$students = $wpdb->get_results($wpdb->prepare(
			'SELECT sr.ID, sr.name as label, sr.enrollment_number, sr.roll_number, sr.phone, sr.father_name, sr.father_phone
			FROM ' . WLSM_STUDENT_RECORDS . ' as sr
			WHERE sr.section_id = %d AND sr.is_active = 1',
			$section_id
		));
		return $students;
	}
	...
	public static function get_students_by_subject($school_id, $session_id, $section_id, $subject_id) {
		global $wpdb;
		$students = $wpdb->get_results($wpdb->prepare(
			...
			JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
			JOIN ' . WLSM_STUDENTS_SUBJECTS . ' as ssb ON ssb.student_id = sr.ID
			WHERE cs.school_id = %d AND ss.ID = %d AND se.ID = %d AND ssb.subject_id = %d
			GROUP BY sr.ID
			ORDER BY sr.roll_number ASC, sr.name ASC',
			$school_id, $session_id, $section_id, $subject_id
		), OBJECT_K);
	...
	public static function get_section_student($school_id, $session_id, $section_id, $student_id)
	{
		...
		$students = $wpdb->get_results($wpdb->prepare('SELECT sr.ID, sr.name, sr.enrollment_number, sr.roll_number, sr.phone, sr.father_name, sr.father_phone, se.label as section_label FROM ' . WLSM_STUDENT_RECORDS . ' as sr
			...
			JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
			WHERE cs.school_id = %d AND ss.ID = %d AND se.ID = %d AND sr.ID = %d' . ' GROUP BY sr.ID ORDER BY sr.roll_number ASC, sr.name ASC', $school_id, $session_id, $section_id, $student_id), OBJECT_K);
		return $students;
	}
	...
	{
		global $wpdb;
		$notice = $wpdb->get_row($wpdb->prepare('SELECT n.ID, n.title, n.description, n.attachment, n.url, n.link_to, n.is_active, n.notice_data FROM ' . WLSM_NOTICES . ' as n
	...
	{
		global $wpdb;
		$sql = 'SELECT n.ID, n.title, n.attachment, n.url, n.link_to, n.is_active, n.created_at, n.notice_data FROM ' . WLSM_NOTICES . ' as n
		JOIN ' . WLSM_CLASS_SCHOOL_NOTICE . ' as csn ON csn.notice_id = n.ID
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = csn.class_school_id
		WHERE cs.class_id = %d AND n.is_active = 1 GROUP BY n.ID ORDER BY n.ID DESC';
		if ($limit) {
			$sql .= (' LIMIT ' . absint($limit));
		}
		$notices = $wpdb->get_results($wpdb->prepare($sql, $class_school_id, $school_id));
		return $notices;
	}
	...
	public static function fetch_subject_query($school_id, $restrict_to_section)
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND cs.default_section_id = ' . absint( $restrict_to_section );
		} else {
	...
	public static function fetch_subject_query_count($school_id, $restrict_to_section)
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND cs.default_section_id = ' . absint( $restrict_to_section );
		} else {
	...
	public static function fetch_admin_subjects($school_id, $user_id)
	{
		global $wpdb;
		$admin_subjects = $wpdb->get_col($wpdb->prepare('SELECT asj.subject_id FROM ' . WLSM_ADMIN_SUBJECT . ' as asj
		JOIN ' . WLSM_SUBJECTS . ' as sj ON sj.ID = asj.subject_id
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = sj.class_school_id
		JOIN ' . WLSM_ADMINS . ' as a ON a.ID = asj.admin_id
		JOIN ' . WLSM_STAFF . ' as sf ON sf.ID = a.staff_id
	...
	public static function get_Assigned_Subjects($adminId ) {
		global $wpdb;
		$query = $wpdb->prepare("
        SELECT s.ID, s.label, s.code, s.type
        FROM " . WLSM_ADMIN_SUBJECT . " as admin_subject
        JOIN " . WLSM_SUBJECTS . " as s ON admin_subject.subject_id = s.ID
        WHERE admin_subject.admin_id = %d
    ", $adminId);
	...
	public static function get_active_admins_ids_in_school($school_id, $admin_ids)
	{
		...
		// Return empty array if no admin IDs provided
	...
	public static function fetch_study_material_query($school_id) {
		$query = 'SELECT DISTINCT sm.ID, sm.label as title, sm.description, c.label as class_label, ss.label as subject_label, sm.attachments, sm.created_at, u.user_login as username
		FROM ' . WLSM_STUDY_MATERIALS . ' as sm
		...
		LEFT OUTER JOIN ' . WLSM_SUBJECTS . ' as ss ON ss.ID = ssm.study_material_subject_id
	...
	public static function fetch_study_material_query_by_admin_id($school_id, $admin_id) {
		$query = 'SELECT sm.ID, sm.label as title, sm.description, c.label as class_label, ss.label as subject_label, sm.attachments, sm.created_at, u.user_login as username
		FROM ' . WLSM_STUDY_MATERIALS . ' as sm
		JOIN ' . WLSM_CLASS_SCHOOL_STUDY_MATERIAL . ' as ssm ON ssm.study_material_id = sm.ID
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = sm.school_id
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = ssm.class_school_id
		LEFT OUTER JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		LEFT OUTER JOIN ' . WLSM_SUBJECTS . ' as ss ON ss.ID = ssm.study_material_subject_id
		JOIN ' . WLSM_ADMIN_SUBJECT . ' as asj ON asj.subject_id = ss.ID AND asj.admin_id = '.absint($admin_id).'
		LEFT OUTER JOIN ' . WLSM_USERS . ' as u ON u.ID = sm.added_by
		WHERE s.ID = ' . absint($school_id);
		return $query;
	}
	...
	public static function fetch_study_material_query_count($school_id, $restrict_to_section = false)
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND ssm.study_material_section_id = ' . absint( $restrict_to_section );
		} else {
			$section_where = '';
		}
		...
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = sm.school_id
		LEFT OUTER JOIN ' . WLSM_USERS . ' as u ON u.ID = sm.added_by
		WHERE s.ID = ' . absint($school_id) . $section_where;
		return $query;
	}
	...
	{
		global $wpdb;
		$study_material = $wpdb->get_row($wpdb->prepare('SELECT sm.ID, sm.label as title,
			c.label as class_label,
			ss.ID as subject_id,
			ssm.study_material_section_id,
			wl.label as section_label,
			c.ID as class_id,
	...
	{
		$query = 'SELECT mt.ID, mt.host_id, mt.meeting_id, mt.moderator_code, mt.password, mt.recordable, mt.topic, mt.duration, mt.start_at, mt.type, mt.class_type, mt.start_url, mt.join_url, c.id as class_id, sj.label as subject_name, sj.code as subject_code, c.label as class_label, a.name as name FROM ' . WLSM_MEETINGS . ' as mt
	...
	{
		$query = 'SELECT mt.ID, mt.host_id, mt.meeting_id, mt.topic, mt.duration, mt.recordable, mt.password, mt.moderator_code, mt.start_at, mt.start_url, mt.type, mt.class_type, mt.join_url, sj.label as subject_name, sj.code as subject_code, c.label as class_label FROM ' . WLSM_MEETINGS . ' as mt
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = mt.school_id
		LEFT OUTER JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = mt.class_school_id
		LEFT OUTER JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		LEFT OUTER JOIN ' . WLSM_SUBJECTS . ' as sj ON sj.ID = mt.subject_id
	...
	public static function fetch_homework_query($school_id, $session_id, $restrict_to_section = false)
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND hwse.section_id = ' . absint( $restrict_to_section );
		} else {
			$section_where = '';
		}

		$query = 'SELECT hw.ID, hw.title, hw.description, hw.homework_date, hw.homework_due_date, c.label as class_label, u.user_login as username FROM ' . WLSM_HOMEWORK . ' as hw
		...
		LEFT OUTER JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		WHERE s.ID = ' . absint($school_id) . ' AND ss.ID = ' . absint($session_id) . $section_where;
		return $query;
	}
	...
	public static function fetch_homework_query_count($school_id, $session_id, $restrict_to_section = false)
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND hwse.section_id = ' . absint( $restrict_to_section );
		} else {
	...
	{
		global $wpdb;
		$homework = $wpdb->get_row($wpdb->prepare('SELECT hw.ID, hw.title, hw.subject, hw.description, hw.downloadable, hw.attachments, hw.attachment_url, hw.homework_date, hw.homework_due_date, c.ID as class_id, cs.ID as class_school_id FROM ' . WLSM_HOMEWORK . ' as hw
			JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = hw.school_id
			JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = hw.session_id
			LEFT OUTER JOIN ' . WLSM_HOMEWORK_SECTION . ' as hwse ON hwse.homework_id = hw.ID
			LEFT OUTER JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = hwse.section_id
	...
	public static function fetch_active_students_of_classes($school_id, $session_id, $class_ids)
	{
		...
		// Return empty array if no class IDs provided
	...
	public static function fetch_time_table_by_class_id($school_id, $class_id) {
		 $query = 'SELECT se.ID, se.label AS section_label, c.label AS class_label, sj.label AS subject_label, rt.room_number, rt.start_time, rt.end_time, rt.day
	...
	// public static function fetch_staff_time_table_by_class_id($school_id, $class_id, $admin_subjects) {

	// 	$admin_subjects = array_map('absint', $admin_subjects);
	// 	$admin_subjects_list = implode(',', $admin_subjects);
	...
	// 		JOIN ' . WLSM_CLASS_SCHOOL . ' AS cs ON cs.ID = se.class_school_id
	// 		JOIN ' . WLSM_SUBJECTS . ' AS sj ON sj.ID = rt.subject_id
	// 		JOIN ' . WLSM_ADMIN_SUBJECT . ' AS asj ON sj.ID = asj.subject_id
	// 		JOIN ' . WLSM_CLASSES . ' AS c ON c.ID = cs.class_id
	// 		WHERE cs.school_id = ' . absint($school_id) . ' AND c.ID = ' . absint($class_id) . ' AND rt.subject_id IN (' . $admin_subjects_list . ')';
    // 	return $query;
	// }

	public static function fetch_staff_time_table_by_class_id($school_id, $section_id, $user_id) {

		 $query = 'SELECT rt.ID, se.ID as section_id, se.label AS section_label, c.label AS class_label, sj.label AS subject_label, rt.room_number, rt.start_time, rt.end_time, rt.day
			FROM ' . WLSM_ROUTINES . ' AS rt
			JOIN ' . WLSM_SECTIONS . ' AS se ON se.ID = rt.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' AS cs ON cs.ID = se.class_school_id
			JOIN ' . WLSM_SUBJECTS . ' AS sj ON sj.ID = rt.subject_id
			...
			JOIN ' . WLSM_STAFF . ' as ws ON ws.ID = wa.staff_id
			JOIN ' . WLSM_USERS . ' as wu ON wu.ID = ws.user_id
			WHERE cs.school_id = ' . absint($school_id) . ' AND rt.section_id = ' . absint($section_id) . ' AND wu.ID = ' . absint( $user_id );
    	return $query;
	}
	...
	public static function fetch_time_table_by_class_id_count( $school_id, $class_id )
	{
		$query = 'SELECT COUNT(DISTINCT se.ID) FROM ' . WLSM_ROUTINES . ' as rt
		JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = rt.section_id
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
		JOIN ' . WLSM_SUBJECTS . ' as sj ON sj.class_school_id = cs.ID
		JOIN ' . WLSM_ADMIN_SUBJECT . ' as asj ON asj.subject_id = sj.ID
		JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		WHERE cs.school_id = ' . absint($school_id).' AND c.ID = '.absint($class_id);
		return $query;
	}
	...
	public static function fetch_staff_time_table_by_class_id_count($school_id, $section_id, $user_id) {

		 $query = 'SELECT COUNT(se.ID) FROM ' . WLSM_ROUTINES . ' AS rt
			JOIN ' . WLSM_SECTIONS . ' AS se ON se.ID = rt.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' AS cs ON cs.ID = se.class_school_id
			JOIN ' . WLSM_SUBJECTS . ' AS sj ON sj.ID = rt.subject_id
			JOIN ' . WLSM_CLASSES . ' AS c ON c.ID = cs.class_id
			...
			JOIN ' . WLSM_STAFF . ' as ws ON ws.ID = wa.staff_id
			JOIN ' . WLSM_USERS . ' as wu ON wu.ID = ws.user_id
			WHERE cs.school_id = ' . absint($school_id) . ' AND se.ID = ' . absint($section_id) . ' AND wu.ID = ' . absint( $user_id );
    	return $query;
	}
	...
	public static function get_date_text( $date ) {
		if ( $date ) {
			return date_format( date_create( $date ), WLSM_Config::date_format() );
		}
		return '';
	}
	...
	public static function get_salary_text( $salary, $currency ) {
		if ( $salary ) {
	...
	public static function get_day_text($day)
	{
		$days = array(
			...
			'6' => esc_html__('Saturday', 'school-management'),
			'7' => esc_html__('Sunday', 'school-management')
		);

		if ( in_array($day, array_keys($days)) ) {
			return $days[$day];
		}
		return esc_html__('-', 'school-management');
	}
	...
	public static function get_class_school_id( $school_id, $class_id ) {
		global $wpdb;
		$data = $wpdb->get_var($wpdb->prepare('SELECT cs.ID FROM ' . WLSM_CLASS_SCHOOL . ' as cs
		WHERE cs.school_id = %d AND cs.class_id = %d', $school_id, $class_id ));
		return absint($data);
	}

	public static function check_subject_exists( $class_school_id, $label) {
		global $wpdb;
		$subject = $wpdb->get_row($wpdb->prepare('SELECT sj.ID FROM ' . WLSM_SUBJECTS . ' as sj WHERE sj.class_school_id = %d AND sj.label = %s', $class_school_id, $label));
		return $subject;
	}
}
