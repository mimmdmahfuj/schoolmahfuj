...
class WLSM_Log {
	...
	public static function save( $school_id, $key, $value, $group = '', $section_id = NULL ) {
		// Logs settings.
	...
	public static function login_record( $user_login ) {
		...
		try {
			...
			$student = $wpdb->get_row(
				$wpdb->prepare( 'SELECT sr.ID, sr.name as student_name, se.ID as section_id, c.label as class_label, se.label as section_label, u.user_login as username, s.ID as school_id, ss.label as session_label FROM ' . WLSM_STUDENT_RECORDS . ' as sr 
			...
			if ( $student ) {
				...
				self::save( $school_id, self::LOGIN, $log_value, 'student', $section_id );
				...
				if ( count( $unique_student_ids ) ) {
					...
					foreach ( $school_ids as $school_id ) {
						...
						self::save( $school_id, self::LOGIN, $log_value, 'parent', $section_id );
					}
				}
			}
			...
			if ( $current_school = $user_info['current_school'] ) {
				...
				if ( in_array( $role, array_keys( $roles ) ) ) {
					...
					self::save( $school_id, self::LOGIN, $log_value, strtolower( $role_label ), $section_id );
				}
			}
	...
	public static function fetch_log_query( $school_id, $restrict_to_section = false ) 
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND lg.section_id = ' . absint( $restrict_to_section );
		} else {
			$section_where = '';
		}
		...
		WHERE lg.school_id = ' . absint( $school_id ) . ' AND lg.log_key != "' . self::ERROR . '"' . $section_where;
		return $query;
	}
	...
	public static function fetch_log_query_count( $school_id, $restrict_to_section = false ) 
	{
		if ( $restrict_to_section ) {
			$section_where = ' AND lg.section_id = ' . absint( $restrict_to_section );
		} else {
