		...
		<table class="table table-bordered wlsm-student-pending-invoices-table wlsm-w-100">
			...
			<tbody>
				...
				foreach ( $invoices as $row ) {
					...
					$query = 'SELECT p.ID, p.receipt_number, p.amount, p.payment_method, p.transaction_id, p.attachment, p.created_at, p.note, p.invoice_label, p.invoice_payable, p.invoice_id FROM ' . WLSM_PENDING_PAYMENTS . ' as p';
					$query .= ' WHERE p.invoice_id = ' . $invoice_id;

					$pending_payments = $wpdb->get_results( $query );

					// $pending_payments_id = $pending_payments[0]->ID;
				?>
				<tr>
					...
					<td class="text-nowrap">
						...
						if ( WLSM_M_Invoice::get_paid_key() !== $row->status ) {
							if ($pending_payments) {
								echo esc_html__( 'Verification Pending', 'school-management' );
							} else {
								echo wp_kses(
