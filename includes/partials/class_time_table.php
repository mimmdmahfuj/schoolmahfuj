				...
				<td>
					<div class="clearfix">
						...
						<?php if (isset($from_staff)) {
							// Check permissions.
							$edit_permission = WLSM_M_Role::can('edit_timetable');
							$delete_permission = WLSM_M_Role::can('delete_timetable');

							// Generate edit link based on permissions.
							$edit_link = $edit_permission ?
								'<a class="text-primary" href="' . esc_url($page_url . "&action=save&id=" . $routine->ID) . '"><span class="dashicons dashicons-edit wlsm-font-large"></span></a>' :
								'<a class="text-muted" href="#"><span class="dashicons dashicons-edit wlsm-font-large"></span></a>';

							// Generate delete link based on permissions.
							$delete_link = $delete_permission ?
								'<a class="text-danger wlsm-delete-routine" data-routine="' . esc_attr($routine->ID) . '" data-nonce="' . esc_attr(wp_create_nonce('delete-routine-' . $routine->ID)) . '" data-message-title="' . esc_attr__('Please Confirm!', 'school-management') . '" data-message-content="' . esc_attr__('This will delete the routine.', 'school-management') . '" data-cancel="' . esc_attr__('Cancel', 'school-management') . '" href="#" data-submit="' . esc_attr__('Confirm', 'school-management') . '"><span class="dashicons dashicons-trash wlsm-font-large"></span></a>' :
								'<a class="text-muted" href="#"><span class="dashicons dashicons-trash wlsm-font-large"></span></a>';
						?>
