...
class WLSM_School {
	public static function fetch_schools() {
		...
		if ( count( $filter_rows_limit ) ) {
			foreach ( $filter_rows_limit as $row ) {
				...
				// Disable delete for protected demo school
				if ( $row->label === 'Intechno Academy Demo' ) {
					$delete_action = '<span class="text-muted" title="' . esc_attr__( 'This demo school cannot be deleted.', 'school-management' ) . '"><span class="dashicons dashicons-lock"></span></span>';
				}
	...
	public static function delete() {
		...
		try {
			...
			// Prevent deletion of protected demo school
			if ( $school->label === 'Intechno Academy Demo' ) {
				throw new Exception( esc_html__( 'This demo school cannot be deleted.', 'school-management' ) );
			}
	...
	public static function assign_classes() {
		...
		try {
			...
			if ( ! $school ) {
				ob_end_clean();
				wp_send_json_error( esc_html__( 'School not found.', 'school-management' ) );
			}
			...
			if ( ! count( $classes ) ) {
				$errors['classes'] = esc_html__( 'Please select atleast one class to assign.', 'school-management' );
			}
			// End validation.

			// Clean the buffer before proceeding
			$buffer = ob_get_clean();
			if ( ! empty( $buffer ) ) {
				wp_send_json_error( 'Unexpected output: ' . $buffer );
			}
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				if ( ! empty( $buffer ) ) {
					$wpdb->query( 'ROLLBACK;' );
					wp_send_json_error( 'Database error: ' . $buffer );
				}
