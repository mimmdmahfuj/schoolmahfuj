			...
			<div class="col-md-12">
				<ul class="nav nav-pills mb-3 mt-1" id="wlsm-settings-tabs" role="tablist">
					...
					<li class="nav-item ml-1">
						<a class="nav-link border border-primary" id="wlsm-appearances-tab" data-toggle="tab" href="#wlsm-appearances" role="tab" aria-controls="wlsm-appearances" aria-selected="true">
							<?php esc_html_e('Appearances', 'school-management'); ?>
						</a>
					</li>
					<li class="nav-item ml-1">
					...
					<li class="nav-item ml-1">
						<a class="nav-link border border-primary" id="wlsm-demo-data-tab" data-toggle="tab" href="#wlsm-demo-data" role="tab" aria-controls="wlsm-demo-data" aria-selected="true">
							<?php esc_html_e('Demo Data', 'school-management'); ?>
						</a>
					</li>
				</ul>
			</div>
		...
		<div class="row">
			<div class="col-md-8">
				<div class="tab-content" id="wlsm-tabs">
					...
					<div class="ml-1 tab-pane fade" id="wlsm-appearances" role="tabpanel" aria-labelledby="wlsm-appearances-tab">

						<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post" id="wlsm-save-appearance-settings-form">
							<?php
							$nonce_action = 'save-appearance-settings';
							$nonce        = wp_create_nonce($nonce_action);
							?>
							<input type="hidden" name="<?php echo esc_attr($nonce_action); ?>" value="<?php echo esc_attr($nonce); ?>">

							<input type="hidden" name="action" value="wlsm-save-appearance-settings">

							<div class="row">
								<div class="col-md-4">
									<label for="wlsm_sidebar_color_enable" class="wlsm-font-bold">
										<?php esc_html_e('Enable Color Scheme', 'school-management'); ?>:
									</label>
								</div>
								<div class="col-md-8">
									<div class="form-group">
										<input <?php checked($sidebar_color_enable, 1, true); ?> class="form-check-input mt-1" type="checkbox" name="sidebar_color_enable" id="wlsm_sidebar_color_enable" value="1">
										<label class="ml-4 mb-1 form-check-label wlsm-font-bold text-secondary" for="wlsm_sidebar_color_enable">
											<?php esc_html_e('Enable custom color scheme', 'school-management'); ?>
										</label>
									</div>
								</div>
							</div>
							...
							<div class="row">
								<div class="col-md-4">
									<label for="wlsm_theme_color" class="wlsm-font-bold">
										<?php esc_html_e('Color Scheme', 'school-management'); ?>:
									</label>
								</div>
								<div class="col-md-8">
									<div class="form-group">
										<select name="theme_color" id="wlsm_theme_color" class="form-control">
											<option <?php selected($theme_color, 'blue', true); ?> value="blue"><?php esc_html_e('Blue', 'school-management'); ?></option>
											<option <?php selected($theme_color, 'green', true); ?> value="green"><?php esc_html_e('Green', 'school-management'); ?></option>
											<option <?php selected($theme_color, 'red', true); ?> value="red"><?php esc_html_e('Red', 'school-management'); ?></option>
											<option <?php selected($theme_color, 'purple', true); ?> value="purple"><?php esc_html_e('Purple', 'school-management'); ?></option>
											<option <?php selected($theme_color, 'orange', true); ?> value="orange"><?php esc_html_e('Orange', 'school-management'); ?></option>
											<option <?php selected($theme_color, 'dark', true); ?> value="dark"><?php esc_html_e('Dark', 'school-management'); ?></option>
										</select>
									</div>
								</div>
							</div>
							...
							<div>
								<span class="float-md-right">
									<button type="submit" class="btn btn-primary" id="wlsm-save-appearance-settings-btn">
										<i class="fas fa-save"></i>&nbsp;
										<?php esc_html_e('Save', 'school-management'); ?>
									</button>
								</span>
					...
					<div class="tab-pane fade" id="wlsm-demo-data" role="tabpanel" aria-labelledby="wlsm-demo-data-tab">
						<h3><?php esc_html_e('Generate Demo Data', 'school-management'); ?></h3>
						<hr>

						<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post" id="wlsm-generate-demo-data-form">
							<?php
							$nonce_action = 'generate-demo-data';
							$nonce        = wp_create_nonce($nonce_action);
							?>
							<input type="hidden" name="<?php echo esc_attr($nonce_action); ?>" value="<?php echo esc_attr($nonce); ?>">
							<input type="hidden" name="action" value="wlsm-generate-dummy-data">

							<div class="mb-3 mt-1 alert alert-info">
								<?php esc_html_e('This will generate demo data for testing purposes.', 'school-management'); ?>
							</div>

							<div class="mt-3 text-right">
								<button type="submit" class="btn btn-primary" id="wlsm-generate-demo-data-btn">
									<i class="fas fa-database"></i>&nbsp;
									<?php esc_html_e('Generate', 'school-management'); ?>
								</button>
							</div>
						</form>

						<hr>
						<h3><?php esc_html_e('Clean Demo Data', 'school-management'); ?></h3>
						<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post" id="wlsm-cleanup-demo-data-form">
							<?php
							$nonce_action = 'cleanup-demo-data';
							$nonce        = wp_create_nonce($nonce_action);
							?>
							<input type="hidden" name="<?php echo esc_attr($nonce_action); ?>" value="<?php echo esc_attr($nonce); ?>">
							<input type="hidden" name="action" value="wlsm-cleanup-demo-data">

							<div class="mb-3 mt-1 alert alert-warning">
								<?php esc_html_e('This will clean up all demo data. This action cannot be undone.', 'school-management'); ?>
							</div>

							<div class="mt-3 text-right">
								<button type="submit" class="btn btn-danger" id="wlsm-cleanup-demo-data-btn">
									<i class="fas fa-trash"></i>&nbsp;
									<?php esc_html_e('Clean Up', 'school-management'); ?>
								</button>
							</div>
						</form>
					</div>
