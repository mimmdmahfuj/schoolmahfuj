...
$page_url_income   = admin_url('admin.php?page=' . WLSM_MENU_STAFF_INCOME);
		...
		if (WLSM_M_Role::check_permission(array('view_students'), $current_school['permissions'])) {
			// Total Students.
		...
		if (WLSM_M_Role::check_permission(array('view_exams'), $current_school['permissions'])) {
		...
		if (WLSM_M_Role::check_permission(array('view_inquiries'), $current_school['permissions'])) {
			// Total Inquiries.
		...
		if (WLSM_M_Role::check_permission(array('manage_admissions', 'view_invoices', 'view_income', 'view_expenses'), $current_school['permissions'])) {
...
<div class="wlsm container-fluid">
			...
			<div class="">
				<div class="card-body">
					<div class="row align-items-end">
						<div class="col-md-3">
							<label for="wlsm_global_accounting_start_date" class="form-label"><?php esc_html_e('Start Date', 'school-management'); ?>:</label>
							<input type="date" id="wlsm_global_accounting_start_date" class="form-control" value="<?php echo esc_attr($session_start_date); ?>">
						</div>
						<div class="col-md-3">
							<label for="wlsm_global_accounting_end_date" class="form-label"><?php esc_html_e('End Date', 'school-management'); ?>:</label>
							<input type="date" id="wlsm_global_accounting_end_date" class="form-control" value="<?php echo esc_attr($session_end_date); ?>">
						</div>
						<div class="col-md-3">
							<button type="button" id="wlsm_update_accounting_data" class="btn btn-primary">
								<i class="fas fa-sync-alt"></i>
								<?php esc_html_e('Update Data', 'school-management'); ?>
							</button>
						</div>
						<div class="col-md-6">
							<small class="text-muted"><?php esc_html_e('Filter applies to both statistics and chart below', 'school-management'); ?></small>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="row mt-1 wlsm-stats-blocks" id="wlsm-accounting-stats-container">
		<!-- Initial stats load -->
		<?php
		// Load the stats cards initially
		$initial_stats_loaded = true;
		?>
				...
				<div class="wlsm-stats-block">
					...
					<div class="wlsm-stats-label">
						...
						printf(
							wp_kses(
								/* translators: %s: session label */
								__('Donation <br><small class="text-secondary"> - Session: %s</small>', 'school-management'),
								array('small' => array('class' => array()), 'br' => array())
							),
							esc_html(WLSM_M_Session::get_label_text($current_session['label']))
						);
		...
		<!-- Chart Section -->
	<?php if (WLSM_M_Role::check_permission(array('view_invoices', 'view_expenses'), $current_school['permissions'])) { ?>
	<div class="row mt-4 mb-4">
		<div class="col-md-12">
			<?php require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/groups/accounting/chart_payment_expense.php'; ?>
		</div>
	</div>
	<?php } ?>

	<?php
	// Add JavaScript for handling global date filter
	$nonce_stats = wp_create_nonce('fetch-accounting-stats');
	$js_accounting = <<<EOT
	(function($) {
		'use strict';
		$(document).ready(function() {

			// Update accounting data when button clicked
			$('#wlsm_update_accounting_data').on('click', function() {
				...
				// Update stats cards
				updateAccountingStats(startDate, endDate);

				// Trigger chart update
				$(document).trigger('accounting_dates_updated');

				setTimeout(function() {
					btn.prop('disabled', false).html('<i class="fas fa-sync-alt"></i> Update Data');
				}, 1000);
			});

			function updateAccountingStats(startDate, endDate) {
				if (!startDate || !endDate) {
					console.log('No dates provided for stats update');
					return;
				}

				// Show loading state
				$('#wlsm-accounting-stats-container').html('<div class="col-12 text-center py-4"><div class="alert alert-info"><i class="fas fa-spinner fa-spin"></i> Loading date-filtered statistics...</div></div>');

				// Make AJAX call to fetch updated stats
				$.ajax({
					url: '$ajax_url',
					type: 'POST',
					data: {
						action: 'wlsm-fetch-accounting-stats',
						nonce: '$nonce_stats',
					...
					success: function(response) {
						if (response.success) {
							...
							// Render the stats HTML returned from server
							if (stats.html) {
								html = stats.html;
							} else {
								html = '<div class="col-12"><div class="alert alert-warning">No statistics data available for the selected date range.</div></div>';
							}

							// Update the container with new stats
							$('#wlsm-accounting-stats-container').html(html);
						} else {
							// Handle error
							$('#wlsm-accounting-stats-container').html('<div class="col-12"><div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> Error loading statistics: ' + (response.data || 'Unknown error') + '</div></div>');
						}
					},
					error: function() {
						$('#wlsm-accounting-stats-container').html('<div class="col-12"><div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> Failed to load date-filtered statistics. Please try again.</div></div>');
					}
				});
			}

			// Auto-load statistics with session dates on page load
			$(document).ready(function() {
				var initialStartDate = $('#wlsm_global_accounting_start_date').val();
				var initialEndDate = $('#wlsm_global_accounting_end_date').val();

				if (initialStartDate && initialEndDate) {
					updateAccountingStats(initialStartDate, initialEndDate);
				}
			});
		});
	})(jQuery);
EOT;
	wp_add_inline_script('wlsm-admin', $js_accounting);
	?>
	...
	<div class="row mt-3 mb-3">
		...
		<?php if (WLSM_M_Role::check_permission(array('view_expenses'), $current_school['permissions'])) { ?>
			<div class="col-md-4 col-sm-6">
				<div class="wlsm-group">
		...
		<?php if (WLSM_M_Role::check_permission(array('view_income'), $current_school['permissions'])) { ?>
			<div class="col-md-4 col-sm-6">
				<div class="wlsm-group">
					<span class="wlsm-group-title"><?php esc_html_e('Donation', 'school-management'); ?></span>
					<div class="wlsm-group-actions">
						...
						<a href="<?php echo esc_url($page_url_income . '&action=save'); ?>" class="btn btn-sm btn-outline-primary">
							<?php esc_html_e('Add New Donation', 'school-management'); ?>
						</a>
						<?php endif; ?>
						<a href="<?php echo esc_url($page_url_income . '&action=category'); ?>" class="btn btn-sm btn-outline-primary">
							<?php esc_html_e('Donation Category', 'school-management'); ?>
						</a>
					</div>
				</div>
			</div>
		...
		<?php if (WLSM_M_Role::check_permission(array('view_fees'), $current_school['permissions'])) { ?>
			<div class="col-md-4 col-sm-6">
				<div class="wlsm-group">
					...
					<div class="wlsm-group-actions">
						...
						<?php if( WLSM_M_Role::can('add_fees')): ?>
