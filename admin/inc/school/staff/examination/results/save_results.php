					...
					<div class="col-md-12">
						<div class="table-responsive w-100">
							<table class="table table-bordered table-striped">
								...
								<tbody>
									<?php 
										$user_can_manage_subjects 	= WLSM_M_Role::can( 'assigned_subjects' );
										$user_can_manage_class 		= WLSM_M_Role::can( 'assigned_class' );

										if( $user_can_manage_subjects && ! $user_can_manage_class ) {
											$admin_subjects = WLSM_M_Staff_Class::fetch_admin_subjects( $school_id, $user_id ); // Example array of teacher's subject IDs
										} else {	
											$admin_subjects = array(); // No subjects assigned to the teacher
										}

										foreach ($exam_papers as $key => $exam_paper) {

											if ( $admin_subjects ) {
												if (!in_array($exam_paper->subject_id, $admin_subjects, true)) {
													continue; // Skip this paper if subject_id is not in the teacher's subjects
												}
											}
