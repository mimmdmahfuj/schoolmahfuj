...
class WLSM_Staff_Import {
	...
	public static function bulk_import_books() {
			$current_user = WLSM_M_Role::can( 'add_library' );
			...
			if ( count( $errors ) < 1 ) {
				try {
					...
					$row = 1;
					$book_id = null; // Initialize book_id
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				while ( $line = fgetcsv( $csv_file ) ) {
					...
					if (!empty($phone) && strlen($phone) > 40) {
						throw new Exception(esc_html__('Maximum length cannot exceed 40 characters.', 'school-management') . WLSM_Import::get_csv_error_msg( $row, 5 ) );
					}

					if (!empty($email)) {
						if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
							throw new Exception(esc_html__('Please provide a valid email.', 'school-management') . WLSM_Import::get_csv_error_msg( $row, 6 ) );
						} elseif (strlen($email) > 60) {
							throw new Exception( esc_html__('Maximum length cannot exceed 60 characters.', 'school-management') . WLSM_Import::get_csv_error_msg( $row, 6 ) );
						}
					}
					...
					if (!empty($designation) && strlen($designation) > 80) {
						throw new Exception( esc_html__('Maximum length cannot exceed 80 characters.', 'school-management') . WLSM_Import::get_csv_error_msg( $row, 11 ) );
					}

					if ( empty( $class_label ) ) {
						throw new Exception( esc_html__( 'Please specify class.', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 12 ) );
					} else {
						// Search class with this label.
						// Checks if class exists in the school.
						$class_school = WLSM_M_Staff_Class::get_class_with_label( $school_id, $class_label );
						if ( ! $class_school ) {
							throw new Exception( esc_html__( 'Class not found.', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 12 ) );
						} else {
							$class_school_id = $class_school->ID;
							$class_id        = $class_school->class_id;
						}
						...
						if ( empty( $section_label ) ) {
							throw new Exception( esc_html__( 'Please specify section.', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 13 ) );
						} else {
							// Search section with this label.
							// Checks if section exists.
							$section = WLSM_M_Staff_Class::get_section_with_label( $school_id, $section_label, $class_school_id );
							...
							if ( ! $section ) {
								throw new Exception( esc_html__( 'Section not found.', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 13 ) );
							} else {
								$section_id = $section->ID;
							}
	...
	public static function bulk_import_attendance() {
		$current_user = WLSM_M_Role::can('edit_attendance');
