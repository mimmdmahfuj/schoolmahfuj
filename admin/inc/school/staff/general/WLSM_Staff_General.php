...
class WLSM_Staff_General
{
	public static function get_class_sections() {
		$current_user = WLSM_M_Role::can(array('manage_admissions', 'view_students', 'view_invoices', 'manage_transfer_student', 'view_certificates', 'view_library', 'view_transport', 'view_homework', 'view_exams', 'manage_admins', 'manage_employees', 'view_student_leaves', 'view_attendance', 'edit_study_materials', 'add_notices', 'add_tickets', 'send_notifications', 'view_admit_cards', 'view_timetable', 'view_live_classes', 'add_lessons', 'edit_homework', 'add_exams', 'assigned_subjects'));
		if (!$current_user) {
			die();
		}
		...
		try {
			...
			$fees = WLSM_M_Staff_Accountant::fetch_fees_by_class_student_type($school_id, $class_id, $student_type);
			$activity = WLSM_M_Staff_General::fetch_class_activity($class_id);


			$assigned_subjects = WLSM_M_Role::can('assigned_subjects');

			if (isset($assigned_subjects) && isset($assigned_subjects['school']) && $assigned_subjects['school']['role'] === 'admin') {
				$subject = WLSM_M_Staff_Class::fetch_subject_query_by_class_id($school_id, $class_id);
			} elseif (isset($assigned_subjects) && isset($assigned_subjects['school']) && $assigned_subjects['school']['role'] !== 'admin' && isset($assigned_subjects['school']['admin_id'])) {
				$admin_id = $assigned_subjects['school']['admin_id'];
				$subject = WLSM_Helper::getAssignedSubjects($admin_id);
			} else {
				// Default fallback - fetch all subjects for the class
				$subject = WLSM_M_Staff_Class::fetch_subject_query_by_class_id($school_id, $class_id);
			}
	...
	public static function get_fee_type(){
		...
		$current_user = WLSM_M_Role::can(array('view_invoices'));
	...
	{
		$current_user = WLSM_M_Role::can(array('view_invoices', 'manage_transfer_student', 'manage_certificates', 'view_library', 'view_student_leaves', 'issue_certificates', 'add_notices', 'add_tickets', 'send_notifications'));
	...
	{
		$current_user = WLSM_M_Role::can(array( 'view_timetable', 'view_live_classes', 'add_lessons', 'edit_homework'));
		...
		try {
			...
			// Check if assigned_subjects permission is enabled
			$current_user = WLSM_M_Role::can('assigned_subjects');
			$role = $current_user['school']['role'];
			if (isset($current_user) &&
				isset($current_user['school']) &&
				$role !== 'admin' &&
				isset($current_user['school']['admin_id'])) {

				$admin_id = $current_user['school']['admin_id'];
				$subjects = WLSM_M_Staff_Class::get_Assigned_Subjects($admin_id);
			} else {
				// Default to fetching all subjects if permissions not set or user is admin
				$subjects = WLSM_M_Staff_Class::get_class_subjects($school_id, $class_id);
			}
	...
	public static function get_class_chapter() {
		$current_user = WLSM_M_Role::can(array( 'view_timetable', 'view_live_classes', 'add_lessons'));
	...
	public static function get_class_subjects_exam() {
		$current_user = WLSM_M_Role::can(array('view_exams'));
	...
	public static function get_class_exams() {
		$current_user = WLSM_M_Role::can(array('view_exam_results'));
	...
	public static function get_class_concessions() {
		$current_user = WLSM_M_Role::can(array('manage_admissions', 'edit_students'));
		...
		try {
			...
			if (!wp_verify_nonce($_POST['nonce'], 'get-class-concessions')) {
				throw new Exception(esc_html__('Security check failed.', 'school-management'));
			}
			...
			// Get concession types for the selected class using helper function
			$concessions = WLSM_M_Staff_Accountant::get_concessions_by_class( $school_id, $class_id );

			$concessions = array_map(function ($concession) use ($school_id) {
				$concession->label = sprintf(
					wp_kses(
						/* translators: 1: concession name, 2: concession details */
						_x('%1$s (%2$s)', 'Concession', 'school-management'),
						array('span' => array('class' => array()))
					),
			...
			wp_send_json_success(array(
				...
				'message' => count($concessions) > 0 ?
					sprintf(esc_html__('%d concession types found for this class.', 'school-management'), count($concessions)) :
					esc_html__('No concession types found for this class.', 'school-management')
			));
	...
	public static function get_class_admit_card_exams() {
		$current_user = WLSM_M_Role::can(array('view_admit_cards'));
		...
		try {
			...
			if (!wp_verify_nonce($_POST['nonce'], 'get-class-admit-exams')) {
				throw new Exception(esc_html__('Security check failed.', 'school-management'));
			}
			...
			// Get exams for the selected class using the existing helper method
			$exams = $wpdb->get_results($wpdb->prepare(
				WLSM_M_Staff_Examination::fetch_exam_query_by_class_id($school_id, $class_id) . ' GROUP BY ex.ID',
				array()
			));
	...
	public static function get_class_result_exams() {
		$current_user = WLSM_M_Role::can(array('view_exam_results'));
		...
		try {
			...
			if (!wp_verify_nonce($_POST['nonce'], 'get-class-result-exams')) {
				throw new Exception(esc_html__('Security check failed.', 'school-management'));
			}
	...
	{
		$current_user = WLSM_M_Role::can(array( 'view_timetable', 'view_live_classes'));
	...
	{
		$current_user = WLSM_M_Role::can(array('add_exams'));
	...
	public static function save_student($current_user)
	{
		...
		try {
			...
			$school_general_auto_invoice_allow_partial_payment  = $settings_general['auto_invoice_allow_partial_payment'];
			...
			$parent_signature = (isset($_FILES['parent_signature']) && is_array($_FILES['parent_signature'])) ? $_FILES['parent_signature'] : NULL;
			...
			// Set default values for concession workflow fields (student form only provides concession_id)
			$concession_status = 'pending'; // Default status for new concessions
			$concession_remarks = '';
			$concession_approved_by = 0;
			$concession_approval_date = NULL;
			$concession_rejection_reason = '';

			// Auto-calculate concession amount from concession type
			$concession_amount = 0;
			if ($concession_id) {
				$concession_type_data = WLSM_M_Staff_Accountant::get_concession_type_by_id($concession_id);
				if ($concession_type_data) {
					$concession_amount = $concession_type_data->fixed_amount;
				}
			}
			...
			$dob_in_words   = isset( $_POST['dob_in_words'] ) ? sanitize_text_field( $_POST['dob_in_words'] ) : '';
			...
			$school_address = isset( $_POST['school_address'] ) ? sanitize_text_field( $_POST['school_address'] ) : '';
			$school_class   = isset( $_POST['school_class'] ) ? sanitize_text_field( $_POST['school_class'] ) : '';
			$pass_out_year  = isset( $_POST['pass_out_year'] ) ? sanitize_text_field( $_POST['pass_out_year'] ) : '';

			$pen  				= isset( $_POST['pen'] ) ? sanitize_text_field( $_POST['pen'] ) : '';
			$apaar				= isset( $_POST['apaar'] ) ? sanitize_text_field( $_POST['apaar'] ) : '';
			$father_id_number  	= isset( $_POST['father_id_number'] ) ? sanitize_text_field( $_POST['father_id_number'] ) : '';
			$mother_id_number   = isset( $_POST['mother_id_number'] ) ? sanitize_text_field( $_POST['mother_id_number'] ) : '';
			...
			if ( WLSM_DEMO_MODE ) {
				if ($is_active == 0) {
				$errors['is_active'] = esc_html__('DEMO MODE ACTIVE.', 'school-management');
				}
			}
			...
			if (!in_array($gender, array_keys(WLSM_Helper::gender_list($school_id)))) {
			...
			if (isset($parent_signature['tmp_name']) && !empty($parent_signature['tmp_name'])) {
				if (!WLSM_Helper::is_valid_file($parent_signature, 'attachment')) {
					$errors['parent_signature'] = esc_html__('File type is not supported.', 'school-management');
				}
			}
		...
		if (count($errors) < 1) {
			try {
				...
				if ($student_id) {
					...
					$student_record_data['parent_signature'] = $student->parent_signature;
				}
				...
				if (!empty($parent_signature)) {
					$parent_signature = media_handle_upload('parent_signature', 0);
					if (is_wp_error($parent_signature)) {
						throw new Exception($parent_signature->get_error_message());
					}
					$student_record_data['parent_signature'] = $parent_signature;
				}
				...
				if ($student_id) {
					...
					// Handle concession update for existing student
					if ($concession_id) {
						// Check if student already has a concession record
						$existing_concession = $wpdb->get_row($wpdb->prepare(
							'SELECT ID, concession_type_id, status FROM ' . WLSM_STUDENT_CONCESSION . '
							WHERE student_record_id = %d AND session_id = %d AND school_id = %d
							ORDER BY created_at DESC LIMIT 1',
							$student_id, $session_id, $school_id
						));

						if ($existing_concession) {
							// Update existing concession if concession type is different
							if ($existing_concession->concession_type_id != $concession_id) {
								$concession_update_data = array(
									'concession_type_id' => $concession_id,
									'applied_by' => get_current_user_id(),
									'application_date' => current_time('Y-m-d H:i:s'),
									'status' => $concession_status, // Always pending for new assignments
									'updated_at' => current_time('Y-m-d H:i:s')
								);
								$wpdb->update(WLSM_STUDENT_CONCESSION, $concession_update_data, array('ID' => $existing_concession->ID));
							}
						} else {
							// Create new concession record
							...
							$wpdb->insert(WLSM_STUDENT_CONCESSION, $concession_data);
						}
					} else {
						// Remove concession if none selected
						$wpdb->delete(WLSM_STUDENT_CONCESSION, array(
							'student_record_id' => $student_id,
							'session_id' => $session_id,
							'school_id' => $school_id
						));
					...
					// Save concession assignment if selected
					if ($concession_id) {
						$concession_data = array(
							'student_record_id' => $student_id,
							'concession_type_id' => $concession_id,
							...
							'approved_by' => $concession_approved_by,
							'approval_date' => $concession_approval_date,
							'rejection_reason' => $concession_rejection_reason,
							'created_at' => current_time('Y-m-d H:i:s')
						);
						$wpdb->insert(WLSM_STUDENT_CONCESSION, $concession_data);
					}
				...
				if (count($fee_label)) {
					foreach ($fee_label as $key => $value) {
						...
						if ($is_insert) {
							...
							$invoice_total_amount 		= $student_fee_data['amount'];
							$invoice_discounted_amount 	= $invoice_discount/100 * $invoice_total_amount;
							$invoice_amount 			= $invoice_total_amount - $invoice_discounted_amount;

							$discount_data['amount']           = $invoice_discounted_amount;
							$discount_data['discount_percent'] = $invoice_discount;
							$discount_data['note']             = 'This discount was added on student admission';
							$discount_data['student_id']       = $student_id;

							// Invoice data.
							$invoice_data = array(
								'label'                => $student_fee_data['label'],
								'amount'               => $invoice_amount,
								'invoice_amount_total' => $invoice_total_amount,
								'discount'             => $invoice_discount,
								'date_issued'          => $student_fee_data['created_at'],
								'due_date'             => $student_fee_data['created_at'],
							);

							if( $school_general_auto_invoice_allow_partial_payment ) {
								$invoice_data['partial_payment'] = 1;
							} else {
								$invoice_data['partial_payment'] = 0;
							}
							...
							if($active_on_dashboard[$key] === '1'){
								...
								// Add invoice_id to discount data
								$discount_data['invoice_id'] = $invoice_id;

								$discount_success = $wpdb->insert(WLSM_DISCOUNTS, $discount_data);
							}
					...
					if($active_on_dashboard[$key] !== "1"){
						...
						$invoice_data['invoice_amount_total'] = $amount;
						...
						if($invoice_data['label'] !== null){
							...
							// Add invoice_id to discount data
							$discount_data['invoice_id'] = $invoice_id;
							$discount_data['amount']     = $discounted;
	...
	public static function get_students()
	{
		...
		$can_edit_students = WLSM_M_Role::check_permission(array('edit_students'), $current_school['permissions']);
		...
		if (count($errors) < 1) {
			...
			try {
				...
				// Ordering.
				$columns = array('sr.name', 'sr.name', 'sr.admission_number', 'sr.student_type', 'sr.phone', 'sr.email', 'c.label', 'se.label', 'sr.roll_number', 'sr.is_active', 'sr.father_name', 'sr.father_phone', 'u.user_email', 'u.user_login', 'sr.admission_date', 'sr.enrollment_number', 'sr.from_front');
				if ($gdpr_enable) {
				...
				if (count($filter_rows_limit)) {
					foreach ($filter_rows_limit as $row) {
						...
						$other_data = array(
							...
							($can_edit_students ? '<a class="text-primary" href="' . esc_url($page_url . "&action=save&id=" . $row->ID) . '"><span class="dashicons dashicons-edit"></span></a>' : '') .
							($can_delete_students ? ('&nbsp;&nbsp;
...
?>
	...
	public static function print_id_card() {
		$current_user = WLSM_M_Role::can('view_students', 'view_transport');
	...
	public static function print_admit_cards() {
		$current_user = WLSM_M_Role::can('view_admit_cards');
	...
	public static function save_staff($current_user, $role)
	{
		...
		if (count($errors) < 1) {
			try {
				...
				// Add photo_id to admin_data if photo was uploaded
				if (!empty($photo)) {
					$photo = media_handle_upload('photo', 0);
					if (is_wp_error($photo)) {
						throw new Exception($photo->get_error_message());
					}
					$admin_data['photo_id'] = $photo;
				}
				if ($admin_id) {
					// Update admin.
	...
	{
		$current_user = WLSM_M_Role::can('take_staff_attendance');
	...
	{
		$current_user = WLSM_M_Role::can('view_staff_attendance');
	...
	{
		$current_user = WLSM_M_Role::can('view_staff_leaves');
		...
		if (count($filter_rows_limit)) {
			foreach ($filter_rows_limit as $row) {
				...
				$can_edit = WLSM_M_Role::can('edit_staff_leaves');
				$can_delete = WLSM_M_Role::can('delete_staff_leaves');
				...
				$data[] = array(
					...
					($can_edit ? '<a class="text-primary" href="' . esc_url($page_url . "&action=save&id=" . $row->ID) . '"><span class="dashicons dashicons-edit"></span></a>' : '<a class="text-muted" href="#"><span class="dashicons dashicons-edit"></span></a>') . '&nbsp;&nbsp;' .
					($can_delete ? '<a class="text-danger wlsm-delete-staff-leave" data-nonce="' . esc_attr(wp_create_nonce('delete-staff-leave-' . $row->ID)) . '" data-staff-leave="' . esc_attr($row->ID) . '" href="#" data-message-title="' . esc_attr__('Please Confirm!', 'school-management') . '" data-message-content="' . esc_attr__('This will delete the staff leave.', 'school-management') . '" data-cancel="' . esc_attr__('Cancel', 'school-management') . '" data-submit="' . esc_attr__('Confirm', 'school-management') . '"><span class="dashicons dashicons-trash"></span></a>' : '<a class="text-muted" href="#"><span class="dashicons dashicons-trash"></span></a>')
				);
			}
		}
	...
	{
		$current_user = WLSM_M_Role::can('edit_staff_leaves');
	...
	{
		$current_user = WLSM_M_Role::can('delete_staff_leaves');
	...
	{
		$current_user = WLSM_M_Role::can('view_certificates');
		...
		if (count($filter_rows_limit)) {
			foreach ($filter_rows_limit as $row) {
				...
				// Table columns.
				$edit_permission = WLSM_M_Role::check_permission(array('edit_certificates'), $current_user['school']['permissions']);
				$issue_permission = WLSM_M_Role::check_permission(array('issue_certificates'), $current_user['school']['permissions']);
				$delete_permission = WLSM_M_Role::check_permission(array('delete_certificates'), $current_user['school']['permissions']);

				$edit_link = $edit_permission ?
					'<a class="text-primary" href="' . esc_url($page_url . "&action=save&id=" . $row->ID) . '"><span class="dashicons dashicons-edit"></span></a>' :
					'<a class="text-muted" href="#"><span class="dashicons dashicons-edit"></span></a>';

				$distribute_link = $issue_permission ?
					...
					'<a class="text-muted wlsm-font-bold" href="#">' . esc_html__('Distribute Certificate', 'school-management') . '</a>';

				$delete_link = $delete_permission ?
					...
					'<a class="text-muted" href="#"><span class="dashicons dashicons-trash"></span></a>';
	...
	{
		$current_user = WLSM_M_Role::can('add_certificates');
	...
	{
		$current_user = WLSM_M_Role::can('delete_certificates');
	...
	{
		$current_user = WLSM_M_Role::can('issue_certificates');
	...
	public static function fetch_certificates_distributed()
	{
		...
		if (count($filter_rows_limit)) {
			foreach ($filter_rows_limit as $row) {
				...
				// Edit button
				$edit_button = WLSM_M_Role::can('edit_certificates') ?
					'<a class="text-primary wlsm-font-bold" href="' . esc_url($page_url . "&action=students&certificate_student_id=" . $row->ID) . '"><span class="dashicons dashicons-search"></span></a>&nbsp' :
					'<span class="text-muted"><span class="dashicons dashicons-edit"></span></span>&nbsp;&nbsp;';

				// Delete button
				$delete_button = WLSM_M_Role::can('delete_certificates') ?
	...
	public static function send_notification() {
		...
		$send_firebase = (isset($_POST['send_firebase'])) ? (bool) $_POST['send_firebase'] : false;
		$firebase_title = (isset($_POST['firebase_title'])) ? sanitize_text_field($_POST['firebase_title']) : '';
		$firebase_body_template = (isset($_POST['firebase_body'])) ? wp_kses_post($_POST['firebase_body']) : '';
		...
		try {
			...
			if (!$send_firebase && !$send_email && !$send_sms) {
				throw new Exception(esc_html__('Please send either firebase or email or sms notification.', 'school-management'));
			}

			// Validate each notification method if selected
			if ($send_firebase) {
				if (empty($firebase_title)) {
					$errors['firebase_title'] = esc_html__('Please provide firebase title.', 'school-management');
				}
				if (empty($firebase_body_template)) {
					$errors['firebase_body'] = esc_html__('Please provide firebase body.', 'school-management');
				}
			}
			...
			try {
				...
				foreach ($student_ids as $student_id) {
					...
					if ($send_firebase) {
						...
						$is_enable = $settings_firebase['enable_firebase'];

						if ($is_enable) {
							$token = get_option($student->user_id . '_st_notification');
							if ($token) {
								...
								$firebase_title = str_replace(array_keys($placeholders), array_values($placeholders), $firebase_title);
								$firebase_body = str_replace(array_keys($placeholders), array_values($placeholders), $firebase_body_template);

								$firebase_success = WLSM_Helper::send_firebase_notification($token, $firebase_title, $firebase_body, $school_id);
								if ($firebase_success) {
									$success = true;
								}
							}
					...
					// Schedule email or SMS notification if requested
					if ($send_email || $send_sms) {
						...
						// Schedule the notification to be sent after 30 seconds
						$scheduled = wp_schedule_single_event(time() + 30, 'wlsm_notify_for_custom_message', array($data));
						if ($scheduled !== false) {
							$success = true;
						}
					}
				...
				if ($success) {
					$message = esc_html__('Notifications have been sent or scheduled successfully.', 'school-management');
					$reset = true;
					wp_send_json_success(array('message' => $message, 'reset' => $reset));
				} else {
					$error = esc_html__('No notifications were sent. Please check your configuration.', 'school-management');
	...
	{
		$current_user = WLSM_M_Role::can('view_inquiries');
		...
		$query = WLSM_M_Staff_General::fetch_inquiry_query($school_id, $restrict_to_section);
		...
		// Total query.
		$rows_query = WLSM_M_Staff_General::fetch_inquiry_query_count($school_id, $restrict_to_section);
		...
		if (count($filter_rows_limit)) {
			...
			foreach ($filter_rows_limit as $row) {
				...
				$actions = '';
				if (WLSM_M_Role::check_permission(array('edit_inquiries'), $current_user['school']['permissions'])) {
					$actions .= '<a class="text-primary" href="' . esc_url($page_url . "&action=save&id=" . $row->ID) . '"><span class="dashicons dashicons-edit"></span></a>&nbsp;&nbsp;';
				} else {
					$actions .= '<span class="dashicons dashicons-edit text-muted"></span>&nbsp;&nbsp;';
				}
				if (WLSM_M_Role::check_permission(array('delete_inquiries'), $current_user['school']['permissions'])) {
					$actions .= '<a class="text-danger wlsm-delete-inquiry" data-nonce="' . esc_attr(wp_create_nonce('delete-inquiry-' . $row->ID)) . '" data-inquiry="' . esc_attr($row->ID) . '" href="#" data-message-title="' . esc_attr__('Please Confirm!', 'school-management') . '" data-message-content="' . esc_attr__('This will delete the inquiry.', 'school-management') . '" data-cancel="' . esc_attr__('Cancel', 'school-management') . '" data-submit="' . esc_attr__('Confirm', 'school-management') . '"><span class="dashicons dashicons-trash"></span></a>';
				} else {
					$actions .= '<span class="dashicons dashicons-trash text-muted"></span>';
				}

				array_push($inquiry_data, $actions);
	...
	{
		$current_user = WLSM_M_Role::can('add_inquiries');
	...
	{
		$current_user = WLSM_M_Role::can('delete_inquiries');
	...
	public static function fetch_logs()
	{
		...
		$query = WLSM_Log::fetch_log_query($school_id, $restrict_to_section );
		...
		// Total query.
		$rows_query = WLSM_Log::fetch_log_query_count($school_id, $restrict_to_section );
		...
		try {
			...
			foreach ($student_ids_keys as $student_id) {
				if (isset($students[$student_id])) {
					...
					$new_student_id = $wpdb->insert_id;
					$target_class_id = isset( $new_class_ids[ $student_id ] ) ? absint( $new_class_ids[ $student_id ] ) : $to_class_id;
					...
					// Insert subject on promotion.
					$subjects = WLSM_M_Staff_Class::get_class_subjects( $school_id, $target_class_id );
					if ( $subjects ) {
						foreach ( $subjects as $subject ) {
							...
							$exists = $wpdb->get_var( $wpdb->prepare( 'SELECT ID FROM ' . WLSM_STUDENTS_SUBJECTS . ' WHERE student_id = %d AND subject_id = %d', $new_student_id, $subject_id ) );
							if ( $exists ) {
								continue;
							}
							...
							$success = $wpdb->insert( WLSM_STUDENTS_SUBJECTS, $student_subject_data );
							if ( false === $success ) {
								throw new Exception( $wpdb->last_error );
							}
						}
	...
	public static function fetch_transferred_to_school()
	{
		...
		$can_manage_students = WLSM_M_Role::check_permission(array('manage_admissions'), $current_school['permissions']);
...
<?php
	...
	public static function save_school_general_settings()
	{
		...
		try {
			...
			$generate_before_due_date  	 		= isset($_POST['before_due_date']) ? absint ($_POST['before_due_date']) : 0;
			$generate_after_due_date   	 		= isset($_POST['after_due_date']) ? absint ($_POST['after_due_date']) : 0;
			$auto_invoice_allow_partial_payment = isset($_POST['auto_invoice_allow_partial_payment']) ? (bool) ($_POST['auto_invoice_allow_partial_payment']) : 0;
		...
		if (count($errors) < 1) {
			try {
				...
				$general_data = array(
					...
					'before_due_date'             			=> $generate_before_due_date,
					'after_due_date'              			=> $generate_after_due_date,
	...
	public static function save_school_firebase_settings()

	{
		...
		try {
			...
			if (!wp_verify_nonce($_POST['save-school-firebase-settings'], 'save-school-firebase-settings')) {
				die();
			}

			// Handle form data.
			$json_file = (isset($_FILES['json_file']) && is_array($_FILES['json_file'])) ? $_FILES['json_file'] : NULL;
			$enable_firebase = isset($_POST['enable_firebase']) ? (bool) $_POST['enable_firebase'] : 0;
			$remove_json_file = isset($_POST['remove-json']) ? (bool) $_POST['remove-json'] : 0;
			$errors = array();

			// Validate JSON file if provided.
			if ($json_file) {
				if ($json_file['error'] !== UPLOAD_ERR_OK) {
					$errors['json_file'] = esc_html__('Error uploading the file.', 'school-management');
				}
				$valid = WLSM_Helper::is_json_file($json_file);
				if (!$valid) {
					$errors['json_file'] = esc_html__('Please provide a valid JSON file.', 'school-management');
				}
			}
			...
			$firebase_settings = $wpdb->get_row($wpdb->prepare(
				'SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "firebase"',
				$school_id
			));
			...
			if (!$firebase_settings) {
				// Insert new Firebase settings.
				$wpdb->insert(
					WLSM_SETTINGS,
					array(
						'setting_key' => 'firebase',
						'setting_value' => serialize($firebase_data),
						'school_id' => $school_id,
					)
				);
			} else {

				$existing_data = unserialize($firebase_settings->setting_value);

				if (!empty($existing_data['json_file_path']) && $file_path) {
					unlink($existing_data['json_file_path']); // Delete the old file.
				} elseif (!$file_path) {

					$file_path = $existing_data['json_file_path'];
				}
				if ($remove_json_file) {
					$file_path = NULL;
				}
				$firebase_data['json_file_path'] = $file_path;

				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($firebase_data)),
					array('ID' => $firebase_settings->ID)
				);
			}
			...
			$message = esc_html__('Firebase settings saved successfully.', 'school-management');
			wp_send_json_success(array('message' => $message));
		} catch (Exception $exception) {
	...
	public static function save_school_sms_carrier_settings()
	{
		...
		try {
			...
			$whatsapp_api_key  	= isset($_POST['whatsapp_api_key']) ? $_POST['whatsapp_api_key'] : '';
			...
			$arkesel_api_key = isset($_POST['arkesel_api_key']) ? sanitize_text_field($_POST['arkesel_api_key']) : '';
			$arkesel_sender  = isset($_POST['arkesel_sender']) ? sanitize_text_field($_POST['arkesel_sender']) : '';
			...
			$bulksmsbd_sender  = isset($_POST['bulksmsbd_sender']) ? sanitize_text_field($_POST['bulksmsbd_sender']) : '';
			...
			$egosms_username = isset($_POST['egosms_username']) ? sanitize_text_field($_POST['egosms_username']) : '';
			$egosms_password = isset($_POST['egosms_password']) ? sanitize_text_field($_POST['egosms_password']) : '';
			$egosms_sender  = isset($_POST['egosms_sender']) ? sanitize_text_field($_POST['egosms_sender']) : '';
			...
			$nimratext_username  	= isset($_POST['nimratext_username']) ? sanitize_text_field($_POST['nimratext_username']) : '';
			$nimratext_sender_name 	= isset($_POST['nimratext_sender_name']) ? sanitize_text_field($_POST['nimratext_sender_name']) : '';
			$nimratext_sms_type  	= isset($_POST['nimratext_sms_type']) ? $_POST['nimratext_sms_type'] : '';
			$nimratext_api_key  	= isset($_POST['nimratext_api_key']) ? $_POST['nimratext_api_key'] : '';
			$nimratext_peid  		= isset($_POST['nimratext_peid']) ? $_POST['nimratext_peid'] : '';
		...
		if (count($errors) < 1) {
			try {
				...
				// SMS nimratext.
				$nimratext = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "nimratext"', $school_id));

				$nimratext_data = array(
					'username'  	=> $nimratext_username,
					'sender_name' 	=> $nimratext_sender_name,
					'sms_type'  	=> $nimratext_sms_type,
					'api_key'  		=> $nimratext_api_key,
					'peid'  		=> $nimratext_peid,
				);
				...
				if (!$nimratext) {
					$wpdb->insert(
						WLSM_SETTINGS,
						array(
							'setting_key'   => 'nimratext',
							'setting_value' => serialize($nimratext_data),
							'school_id'     => $school_id,
						)
					);
				} else {
					$wpdb->update(
						WLSM_SETTINGS,
						array('setting_value' => serialize($nimratext_data)),
						array('ID'            => $nimratext->ID)
					);
				}
				...
				// Arkesel.
				$arkesel = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "arkesel"', $school_id));
				...
				if (!$arkesel) {
					$wpdb->insert(
						...
						array(
							'setting_key'   => 'arkesel',
							'setting_value' => serialize($arkesel_data),
							'school_id'     => $school_id,
						)
					);
				} else {
					$wpdb->update(
						WLSM_SETTINGS,
						array('setting_value' => serialize($arkesel_data)),
						array('ID'            => $arkesel->ID)
					);
				}
				...
				// egosms.
				$egosms = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "egosms"', $school_id));
				...
				if (!$egosms) {
					$wpdb->insert(
						...
						array(
							'setting_key'   => 'egosms',
							'setting_value' => serialize($egosms_data),
							'school_id'     => $school_id,
						)
					);
				} else {
					$wpdb->update(
						WLSM_SETTINGS,
						array('setting_value' => serialize($egosms_data)),
						array('ID'            => $egosms->ID)
					);
				}
	...
	public static function save_school_payment_method_settings()
	{
		...
		try {
			...
			$payu_enable        = isset($_POST['payu_enable']) ? (bool) ($_POST['payu_enable']) : 0;
			$payu_merchant_key  = isset($_POST['payu_merchant_key']) ? sanitize_text_field($_POST['payu_merchant_key']) : '';
			$payu_merchant_salt = isset($_POST['payu_merchant_salt']) ? sanitize_text_field($_POST['payu_merchant_salt']) : '';
			$payu_mode          = isset($_POST['payu_mode']) ? sanitize_text_field($_POST['payu_mode']) : 'test';
			...
			$amberpay_enable        = isset($_POST['amberpay_enable']) ? (bool) ($_POST['amberpay_enable']) : 0;
			$amberpay_client_id     = isset($_POST['amberpay_client_id']) ? sanitize_text_field($_POST['amberpay_client_id']) : '';
			$amberpay_signature_url = isset($_POST['amberpay_signature_url']) ? sanitize_text_field($_POST['amberpay_signature_url']) : '';
			$amberpay_payment_url   = isset($_POST['amberpay_payment_url']) ? sanitize_text_field($_POST['amberpay_payment_url']) : '';
			$amberpay_api_key       = isset($_POST['amberpay_api_key']) ? sanitize_text_field($_POST['amberpay_api_key']) : '';
			$amberpay_mode          = isset($_POST['amberpay_mode']) ? sanitize_text_field($_POST['amberpay_mode']) : '';

			// Cash payment method
			$cash_enable = isset($_POST['cash_enable']) ? (bool) ($_POST['cash_enable']) : 0;
			$cash_instructions = isset($_POST['cash_instructions']) ? sanitize_textarea_field($_POST['cash_instructions']) : '';

			// Card payment method
			$card_enable = isset($_POST['card_enable']) ? (bool) ($_POST['card_enable']) : 0;
			$card_instructions = isset($_POST['card_instructions']) ? sanitize_textarea_field($_POST['card_instructions']) : '';

			// Check payment method
			$check_enable = isset($_POST['check_enable']) ? (bool) ($_POST['check_enable']) : 0;
			$check_instructions = isset($_POST['check_instructions']) ? sanitize_textarea_field($_POST['check_instructions']) : '';

			// Demand Draft payment method
			$demand_draft_enable = isset($_POST['demand_draft_enable']) ? (bool) ($_POST['demand_draft_enable']) : 0;
			$demand_draft_instructions = isset($_POST['demand_draft_instructions']) ? sanitize_textarea_field($_POST['demand_draft_instructions']) : '';


			// Cash.
			$cash = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "cash"', $school_id));
			...
			if (!$cash) {
				$wpdb->insert(
					...
					array(
						'setting_key'   => 'cash',
						'setting_value' => serialize($cash_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($cash_data)),
					array('ID'            => $cash->ID)
				);
			}

			// Card.
			$card = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "card"', $school_id));
			...
			if (!$card) {
				$wpdb->insert(
					...
					array(
						'setting_key'   => 'card',
						'setting_value' => serialize($card_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($card_data)),
					array('ID'            => $card->ID)
				);
			}

			// Check.
			$check = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "check"', $school_id));
			...
			if (!$check) {
				$wpdb->insert(
					...
					array(
						'setting_key'   => 'check',
						'setting_value' => serialize($check_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($check_data)),
					array('ID'            => $check->ID)
				);
			}

			// Demand Draft.
			$demand_draft = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "demand_draft"', $school_id));

			$demand_draft_data = array(
				'enable'  => $demand_draft_enable,
				'instructions' => $demand_draft_instructions,
			);
			...
			if (!$demand_draft) {
				$wpdb->insert(
					WLSM_SETTINGS,
					array(
						'setting_key'   => 'demand_draft',
						'setting_value' => serialize($demand_draft_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($demand_draft_data)),
					array('ID'            => $demand_draft->ID)
				);
			}
			...
			if (!in_array($payu_mode, array('test', 'live'))) {
				$payu_mode = 'test';
			}
		...
		try {
			...
			// PayU.
			$payu = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "payu"', $school_id));
			...
			if (!$payu) {
				$wpdb->insert(
					...
					array(
						'setting_key'   => 'payu',
						'setting_value' => serialize($payu_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($payu_data)),
					array('ID'            => $payu->ID)
				);
			}

			// Amberpay.
			$amberpay = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "amberpay"', $school_id));

			$amberpay_data = array(
				'enable'          => $amberpay_enable,
				'client_id'       => $amberpay_client_id,
				'signature_url'   => $amberpay_signature_url,
				'payment_url'     => $amberpay_payment_url,
				'api_key'         => $amberpay_api_key,
				'mode'            => $amberpay_mode,
			);
			...
			if (!$amberpay) {
				$wpdb->insert(
					WLSM_SETTINGS,
					array(
						'setting_key'   => 'amberpay',
						'setting_value' => serialize($amberpay_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($amberpay_data)),
					array('ID'            => $amberpay->ID)
				);
			}
	...
	public static function save_school_registration_settings()
	{
		...
		try {
			...
			$registration_declaration_message            = isset($_POST['registration_declaration_message']) ? sanitize_text_field($_POST['registration_declaration_message']) : '';
			...
			$gender            = isset($_POST['gender']) ? (bool) ($_POST['gender']) : '';
			...
			$student_login     = isset($_POST['registration_student_login']) ? (bool) ($_POST['registration_student_login']) : '';
			...
			$pan               = isset($_POST['registration_pan']) ? (bool) ($_POST['registration_pan']) : '';
			$student_aprove    = isset($_POST['student_aprove']) ? (bool) ($_POST['student_aprove']) : '';

			$registration_dob_in_words   = isset($_POST['registration_dob_in_words']) ? (bool) ($_POST['registration_dob_in_words']) : '';
			$registration_birth_place    = isset($_POST['registration_birth_place']) ? (bool) ($_POST['registration_birth_place']) : '';
			$registration_mother_tongue  = isset($_POST['registration_mother_tongue']) ? (bool) ($_POST['registration_mother_tongue']) : '';
			$registration_school_details = isset($_POST['registration_school_details']) ? (bool) ($_POST['registration_school_details']) : '';
			$registration_category       = isset($_POST['registration_category']) ? (bool) ($_POST['registration_category']) : '';
			$registration_address        = isset($_POST['registration_address']) ? (bool) ($_POST['registration_address']) : '';
			$registration_student_type   = isset($_POST['registration_student_type']) ? (bool) ($_POST['registration_student_type']) : '';
			$registration_activity       = isset($_POST['registration_activity']) ? (bool) ($_POST['registration_activity']) : '';
			$registration_student_photo  = isset($_POST['registration_student_photo']) ? (bool) ($_POST['registration_student_photo']) : '';
			$student_auto_subjects       = isset($_POST['registration_auto_select_subjects']) ? (bool) ($_POST['registration_auto_select_subjects']) : '';

			$pen       					= isset($_POST['registration_pen']) ? (bool) ($_POST['registration_pen']) : '';
			$apaar       				= isset($_POST['registration_apaar']) ? (bool) ($_POST['registration_apaar']) : '';
			$parent_id_number       	= isset($_POST['registration_parent_id_number']) ? (bool) ($_POST['registration_parent_id_number']) : '';
			$add_sibling_button       	= isset($_POST['registration_add_sibling_button']) ? (bool) ($_POST['registration_add_sibling_button']) : '';
			...
			if ( isset( $_POST['gender_list'] ) && ! empty( $_POST['gender_list'] ) ) {
				$submitted_gender_list = json_decode( stripslashes( $_POST['gender_list'] ), true );
				if ( is_array( $submitted_gender_list ) ) {
					$gender_list = array();
					foreach ( $submitted_gender_list as $key => $label ) {
						$key = sanitize_key( $key );
						$label = sanitize_text_field( $label );
						if ( ! empty( $key ) && ! empty( $label ) ) {
							$gender_list[ $key ] = $label;
						}
					}
					// Ensure Male and Female are always present
					if ( ! isset( $gender_list['male'] ) ) {
						$gender_list['male'] = esc_html__( 'Male', 'school-management' );
					}
					if ( ! isset( $gender_list['female'] ) ) {
						$gender_list['female'] = esc_html__( 'Female', 'school-management' );
					}
				}
			}
			...
			$registration_data = array(
				...
				'medium'                => $medium,
				'pan'                   => $pan,
				'dob_in_words'          => $registration_dob_in_words,
				'birth_place'           => $registration_birth_place,
				'mother_tongue'         => $registration_mother_tongue,
				'school_details'        => $registration_school_details,
				'category'              => $registration_category,
				'student_type'          => $registration_student_type,
				'address'               => $registration_address,
				'activity'              => $registration_activity,
				'student_photo'         => $registration_student_photo,
				'student_auto_subjects' => $student_auto_subjects,
				...
				'add_sibling_button'    => $add_sibling_button,
	...
	public static function save_school_dashboard_settings()
	{
		...
		try {
			...
			$registration_auto_select_subjects       = isset($_POST['registration_auto_select_subjects']) ? (bool)($_POST['registration_auto_select_subjects']) : '';
			...
			$dashboard_data = array(
				...
				'student_auto_subjects'   => $registration_auto_select_subjects,
	...
	public static function print_staff_id_cards() {
		$current_user = WLSM_M_Role::can( array( 'manage_employees' ) );
		...
		try {
			...
			if ( ! wp_verify_nonce( $_POST['print-staff-id-cards'], 'print-staff-id-cards' ) ) {
				die();
			}
			...
			$where_conditions[] = 'sf.school_id = ' . absint( $school_id );
			...
			if ( $role_id ) {
				$where_conditions[] = 'a.role_id = ' . absint( $role_id );
			}

			$where_clause = implode( ' AND ', $where_conditions );

			$query = 'SELECT a.ID, a.name, a.phone, a.email, a.designation, a.joining_date, sf.role, a.photo_id, r.name as role_name
					  FROM ' . WLSM_ADMINS . ' as a
			...
			ob_start();
			require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/print/staff_id_cards.php';
			$html = ob_get_clean();

			$json = json_encode( array(
				'message_title' => esc_html__( 'Staff ID Cards', 'school-management' ),
				'message'       => esc_html__( 'Staff ID Cards generated successfully.', 'school-management' ),
			) );
	...
	// Unassign Classes
	public static function unassign_classes() {
		...
		try {
			...
			if (!wp_verify_nonce($_POST['nonce'], 'wlsm-unassign-classes')) {
				throw new Exception(esc_html__('Security check failed.', 'school-management'));
			}

			if (!isset($_POST['class_ids']) || !is_array($_POST['class_ids'])) {
				throw new Exception(esc_html__('Please select at least one class to unassign.', 'school-management'));
			}
			...
			if (!empty($unassigned_classes)) {
				...
				$wpdb->query('ROLLBACK');
				throw new Exception(esc_html__('No classes were unassigned. They may have already been unassigned.', 'school-management'));
			}
	...
	public static function add_gender_option() {
		$current_user = WLSM_M_Role::can('manage_settings');
		WLSM_Helper::check_demo();
		...
		if (!wp_verify_nonce($_POST['nonce'], 'add-gender-option')) {
		...
		try {
			...
			if (empty($key) || empty($label)) {
				throw new Exception(esc_html__('Key and label are required.', 'school-management'));
			}

			// Validate key format
			if (!preg_match('/^[a-z_]+$/', $key)) {
				throw new Exception(esc_html__('Key must contain only lowercase letters and underscores.', 'school-management'));
			}

			// Get current settings
			$settings = WLSM_M_Setting::get_settings_registration($school_id);
			$gender_list = $settings['gender_list'];

			// Check if key already exists
			if (isset($gender_list[$key])) {
				throw new Exception(esc_html__('This gender option already exists.', 'school-management'));
			}
			...
			if ($settings_registration) {
				$settings = unserialize($settings_registration->setting_value);
				$settings['gender_list'] = $gender_list;

				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($settings)),
					array('ID' => $settings_registration->ID)
				);
			}

			wp_send_json_success(array(
				'message' => esc_html__('Gender option added successfully.', 'school-management'),
				'gender_list' => $gender_list
			));
	...
	public static function edit_gender_option() {
		$current_user = WLSM_M_Role::can('manage_settings');
		WLSM_Helper::check_demo();
		...
		if (!wp_verify_nonce($_POST['nonce'], 'edit-gender-option')) {
			wp_send_json_error(esc_html__('Security check failed.', 'school-management'));
		}
		...
		try {
			...
			// Check if key exists
			if (!isset($gender_list[$key])) {
				throw new Exception(esc_html__('Gender option not found.', 'school-management'));
			}
			...
			wp_send_json_success(array(
				'message' => esc_html__('Gender option updated successfully.', 'school-management'),
				'gender_list' => $gender_list
			));
	...
	public static function delete_gender_option() {
		$current_user = WLSM_M_Role::can('manage_settings');
		WLSM_Helper::check_demo();
		...
		if (!wp_verify_nonce($_POST['nonce'], 'delete-gender-option')) {
			wp_send_json_error(esc_html__('Security check failed.', 'school-management'));
		}
		...
		try {
			...
			if (empty($key)) {
				throw new Exception(esc_html__('Key is required.', 'school-management'));
			}

			// Prevent deletion of required gender options
			if (in_array($key, ['male', 'female'])) {
				throw new Exception(esc_html__('Male and Female options cannot be deleted.', 'school-management'));
			}
			...
			wp_send_json_success(array(
				'message' => esc_html__('Gender option deleted successfully.', 'school-management'),
				'gender_list' => $gender_list
			));
	...
	public static function reorder_gender_options() {
		$current_user = WLSM_M_Role::can('manage_settings');
		WLSM_Helper::check_demo();
		...
		if (!wp_verify_nonce($_POST['nonce'], 'reorder-gender-options')) {
			wp_send_json_error(esc_html__('Security check failed.', 'school-management'));
		}
		...
		try {
			...
			if (!is_array($order)) {
				throw new Exception(esc_html__('Invalid order data.', 'school-management'));
			}

			// Get current settings
			$settings = WLSM_M_Setting::get_settings_registration($school_id);
			$current_gender_list = $settings['gender_list'];

			// Reorder gender list
			$new_gender_list = array();
			foreach ($order as $key) {
				$key = sanitize_key($key);
				if (isset($current_gender_list[$key])) {
					$new_gender_list[$key] = $current_gender_list[$key];
				}
			}
			...
			if ($settings_registration) {
				$settings = unserialize($settings_registration->setting_value);
				$settings['gender_list'] = $new_gender_list;
			...
			wp_send_json_success(array(
				'message' => esc_html__('Gender options reordered successfully.', 'school-management'),
	...
	// Transfer Certificate functionality
	public static function transfer_certificate() {
		// Try primary permission first, fallback to general student management
		$current_user = WLSM_M_Role::can('issue_certificates');
		if (!$current_user) {
		...
		try {
			...
			if (!wp_verify_nonce($_POST['transfer-certificate-' . $session_id], 'transfer-certificate-' . $session_id)) {
				throw new Exception(esc_html__('Security check failed. Please reload the page and try again.', 'school-management'));
			}

			// Get form data
			$student_id = isset($_POST['student']) ? absint($_POST['student']) : 0;
			$certificate_ids = isset($_POST['certificate_ids']) && is_array($_POST['certificate_ids']) ? $_POST['certificate_ids'] : array();
			$student_status = isset($_POST['student_status']) ? absint($_POST['student_status']) : 1;
			...
			if (empty($certificate_ids) || !is_array($certificate_ids)) {
				throw new Exception(esc_html__('Please select at least one certificate.', 'school-management'));
			}

			// Sanitize certificate IDs
			$certificate_ids = array_map('absint', $certificate_ids);
			$certificate_ids = array_filter($certificate_ids);

			if (empty($certificate_ids)) {
				throw new Exception(esc_html__('Invalid certificate selection.', 'school-management'));
			}
			...
			// Get certificate number prefix for this school
			$last_certificate_count = $wpdb->get_var($wpdb->prepare(
			...
			// Array to track successfully issued certificates
			$issued_certificate_details = array();
			$failed_certificates = array();

			// Insert transfer certificate record for each certificate
			foreach ($certificate_ids as $certificate_id) {
				// Verify certificate belongs to this school
				$certificate = $wpdb->get_row($wpdb->prepare(
					"SELECT ID, label FROM " . WLSM_CERTIFICATES . " WHERE ID = %d AND school_id = %d",
					$certificate_id, $school_id
				));

				if (!$certificate) {
					$failed_certificates[] = "Certificate ID $certificate_id not found";
				...
				$existing_certificate = $wpdb->get_var($wpdb->prepare(
					"SELECT ID FROM " . WLSM_TRANSFER_CERTIFICATES . "
					WHERE student_record_id = %d AND certificate_id = %d",
					$student_id, $certificate_id
				));

				if ($existing_certificate) {
					$failed_certificates[] = "Certificate '{$certificate->label}' already issued to this student";
					continue;
				}

				// Generate certificate number
				$new_certificate_count = absint($last_certificate_count) + $certificates_issued + 1;
				$certificate_number = 'TC-' . str_pad($new_certificate_count, 6, '0', STR_PAD_LEFT);

				// Insert transfer certificate record
				$data = array(
					'student_record_id' => $student_id,
					'certificate_id' => $certificate_id,
					'school_id' => $school_id,
				...
				$success = $wpdb->insert(WLSM_TRANSFER_CERTIFICATES, $data);

				if (false === $success) {
					error_log('Transfer Certificate Debug - Database Error: ' . $wpdb->last_error);
					$failed_certificates[] = "Failed to issue certificate '{$certificate->label}': " . $wpdb->last_error;
					continue;
				}

				// Distribute certificate to student (add to certificate_student table)
				$distribute_success = $wpdb->insert(
				...
				if (false === $distribute_success) {
					error_log('Transfer Certificate Debug - Distribution Error: ' . $wpdb->last_error);
					// Rollback the transfer certificate record
					$wpdb->delete(WLSM_TRANSFER_CERTIFICATES, array('ID' => $wpdb->insert_id));
					$failed_certificates[] = "Failed to distribute certificate '{$certificate->label}': " . $wpdb->last_error;
					continue;
				}
			...
			// Check if any certificates were successfully issued
			if ($certificates_issued == 0) {
				$error_message = esc_html__('No certificates could be issued.', 'school-management');
				if (!empty($failed_certificates)) {
					$error_message .= ' Errors: ' . implode(', ', $failed_certificates);
				}
				throw new Exception($error_message);
			}

			// Update last certificate count in schools table
			$wpdb->update(
				WLSM_SCHOOLS,
				array('last_certificate_count' => absint($last_certificate_count) + $certificates_issued),
				array('ID' => $school_id)
			);
			...
			if ($student_status == 0) {
				$wpdb->update(
					...
					array('is_active' => 0, 'updated_at' => current_time('Y-m-d H:i:s')),
					array('ID' => $student_id)
				);
			}
			...
			$certificate_names = array_column($issued_certificate_details, 'name');
			$certificate_numbers = array_column($issued_certificate_details, 'number');

			$message = sprintf(
				esc_html__('%d transfer certificate(s) issued successfully to %s:', 'school-management'),
				$certificates_issued,
				$student->student_name
			);

			$message .= ' ' . implode(', ', $certificate_names);

			// Add warning if some certificates failed
			if (!empty($failed_certificates)) {
				$message .= ' ' . sprintf(
					esc_html__('Note: %d certificate(s) could not be issued due to errors.', 'school-management'),
					count($failed_certificates)
				);
			}
			...
			wp_send_json_success(array(
				...
				'student_status_updated' => ($student_status == 0)
			));
	...
	public static function fetch_student_transfer_details() {
		// Try primary permission first, fallback to general student management
		$current_user = WLSM_M_Role::can('view_certificates');
		if (!$current_user) {
			$current_user = WLSM_M_Role::can('manage_students');
		...
		try {
			...
			$basic_info = self::generate_student_basic_info_html($student);

			// Generate accounting details HTML
			$accounting_details = self::generate_student_accounting_details_html($school_id, $session_id, $student_id);

			// Generate exam details HTML
			$exam_details = self::generate_student_exam_details_html($school_id, $session_id, $student_id);
	...
	private static function generate_student_basic_info_html($student) {
		ob_start();
		?>
		<div class="row">
			<div class="col-md-3">
				<?php if ($student->photo_id) : ?>
					<img src="<?php echo esc_url(wp_get_attachment_url($student->photo_id)); ?>" class="img-fluid rounded" style="max-width: 150px;">
				<?php else : ?>
					<div class="bg-light p-3 text-center rounded">
						<i class="fas fa-user fa-3x text-muted"></i>
						<p class="mt-2 mb-0 text-muted"><?php esc_html_e('No Photo', 'school-management'); ?></p>
					</div>
				<?php endif; ?>
			</div>
			<div class="col-md-9">
				<table class="table table-sm table-borderless">
					<tr>
						<td><strong><?php esc_html_e('Name', 'school-management'); ?>:</strong></td>
						<td><?php echo esc_html($student->student_name); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Admission Number', 'school-management'); ?>:</strong></td>
						<td><?php echo esc_html($student->admission_number); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Roll Number', 'school-management'); ?>:</strong></td>
						<td><?php echo esc_html($student->roll_number); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Class', 'school-management'); ?>:</strong></td>
						<td><?php echo esc_html($student->class_label); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Section', 'school-management'); ?>:</strong></td>
						<td><?php echo esc_html($student->section_label); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Father\'s Name', 'school-management'); ?>:</strong></td>
						<td><?php echo esc_html($student->father_name); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Phone', 'school-management'); ?>:</strong></td>
						<td><?php echo esc_html($student->phone); ?></td>
					</tr>
					<tr>
						<td><strong><?php esc_html_e('Email', 'school-management'); ?>:</strong></td>
	...
	private static function generate_student_accounting_details_html($school_id, $session_id, $student_id) {
		ob_start();
				...
				<table class="table table-sm table-bordered">
					...
					<tbody>
						<?php foreach ($invoices as $invoice) :
							$total_payable += $invoice->payable;
							$total_paid += $invoice->paid;
							$total_due += $invoice->due;
						?>
							<tr>
								<td><?php echo esc_html($invoice->invoice_title); ?></td>
								<td><?php echo esc_html(WLSM_Config::get_date_text($invoice->date_issued)); ?></td>
								<td><?php echo esc_html(WLSM_Config::sanitize_money($invoice->payable)); ?></td>
								<td><?php echo esc_html(WLSM_Config::sanitize_money($invoice->paid)); ?></td>
								<td><?php echo esc_html(WLSM_Config::sanitize_money($invoice->due)); ?></td>
								<td>
									<?php
									...
									$status_class = isset($status_classes[$invoice->status]) ? $status_classes[$invoice->status] : 'badge-secondary';
									?>
									<span class="badge ">
										<?php echo (WLSM_M_Invoice::get_status_text($invoice->status)); ?>
									</span>
								</td>
							</tr>
						<?php endforeach; ?>
					...
					<tfoot class="thead-dark">
						<tr>
							...
							<th></th>
							<th><?php echo esc_html(WLSM_Config::sanitize_money($total_payable)); ?></th>
							<th><?php echo esc_html(WLSM_Config::sanitize_money($total_paid)); ?></th>
							<th><?php echo esc_html(WLSM_Config::sanitize_money($total_due)); ?></th>
							<th></th>
						</tr>
					</tfoot>
				</table>
		...
		<?php else : ?>
			<div class="text-center py-3">
				...
				<p class="mt-2 text-muted"><?php esc_html_e('No fee records found for this student.', 'school-management'); ?></p>
			</div>
		<?php endif; ?>
	...
	private static function generate_student_exam_details_html($school_id, $session_id, $student_id) {
		ob_start();
		...
		?>
		<h6 class="text-info"><?php esc_html_e('Examination Records', 'school-management'); ?></h6>
				...
				<table class="table table-sm table-bordered">
					<thead class="thead-light">
						<tr>
							...
							<th><?php esc_html_e('Total Marks', 'school-management'); ?></th>
					...
					<tbody>
						<?php foreach ($exam_results as $exam_result) :
							// Get exam totals for this student using admit card ID
							// Since get_student_published_exam_results returns admit card data, we need to get the exam details
							$admit_card_result = WLSM_M_Staff_Examination::get_student_published_exam_result($school_id, $student_id, $exam_result->ID);

							if ($admit_card_result) {
								$exam_totals = WLSM_M_Staff_Examination::get_exam_results_total_by_student_id($school_id, $admit_card_result->exam_id, $student_id);
								$maximum_marks = $exam_totals ? $exam_totals->total_marks : 0;
								$obtained_marks = $exam_totals ? $exam_totals->obtained_marks : 0;
								$percentage = $maximum_marks > 0 ? round(($obtained_marks / $maximum_marks) * 100, 2) : 0;
							} else {
								$maximum_marks = 0;
								$obtained_marks = 0;
								$percentage = 0;
						...
						?>
							<tr>
								<td><?php echo esc_html($exam_result->exam_title); ?></td>
								<td><?php echo esc_html(WLSM_Config::get_date_text($exam_result->start_date)); ?></td>
								<td><?php echo esc_html(WLSM_Config::get_date_text($exam_result->end_date)); ?></td>
								<td><?php echo esc_html($maximum_marks); ?></td>
								<td><?php echo esc_html($obtained_marks); ?></td>
								<td>
									<span class="<?php echo $percentage >= 50 ? 'text-success' : 'text-danger'; ?> font-weight-bold">
										<?php echo esc_html($percentage . '%'); ?>
									</span>
								</td>
								<td>
									<span class="badge badge-success">
										<?php esc_html_e('Completed', 'school-management'); ?>
									</span>
								</td>
							</tr>
						<?php endforeach; ?>
		...
		<?php else : ?>
			<div class="text-center py-3">
				<i class="fas fa-info-circle fa-2x text-muted"></i>
				<p class="mt-2 text-muted"><?php esc_html_e('No exam records found for this student.', 'school-management'); ?></p>
			</div>
		<?php endif; ?>
	...
	public static function get_transfer_certificate_page_url() {
		return admin_url('admin.php?page=' . WLSM_MENU_STAFF_TRANSFER_CERTIFICATES);
	}
	...
	public static function get_school_certificates($school_id) {
		...
		$certificates = $wpdb->get_results($wpdb->prepare(
			"SELECT c.ID, c.label, c.content
			FROM " . WLSM_CERTIFICATES . " c
			WHERE c.school_id = %d
	...
	public static function fetch_transfer_certificates() {
		$current_user = WLSM_M_Role::can(array('view_certificates'));
		if (!$current_user) {
			die();
		}
		$school_id = $current_user['school']['id'];
		$session_id = $current_user['session']['ID'];
		$permissions = $current_user['school']['permissions'];

		global $wpdb;

		$query = WLSM_M_Staff_General::fetch_transfer_certificate_query($school_id, $session_id);
		$query_filter = $query;
		...
		if (isset($_POST['search']['value'])) {
			...
			if ('' !== $search_value) {
				$conditions = array(
					'(tc.certificate_number LIKE "%' . $search_value . '%")',
					'(sr.name LIKE "%' . $search_value . '%")',
					'(sr.admission_number LIKE "%' . $search_value . '%")',
					'(sr.roll_number LIKE "%' . $search_value . '%")',
					'(cert.label LIKE "%' . $search_value . '%")',
					'(c.label LIKE "%' . $search_value . '%")',
					'(se.label LIKE "%' . $search_value . '%")',
					'(tc.remarks LIKE "%' . $search_value . '%")'
				);

				$date_format = WLSM_Config::date_format();

				$search_value_lowercase = strtolower($search_value);
				if (preg_match('/^inac(|t|ti|tiv|tive)$/', $search_value_lowercase)) {
					$conditions[] = '(sr.is_active = 0)';
					$conditions[] = '(tc.student_status = 0)';
				} elseif (preg_match('/^acti(|v|ve)$/', $search_value_lowercase)) {
					$conditions[] = '(sr.is_active = 1)';
					$conditions[] = '(tc.student_status = 1)';
				}

				$issued_date = DateTime::createFromFormat($date_format, $search_value);
				$format_issued_date = $issued_date ? 'Y-m-d' : null;

				if (!$issued_date) {
					if ('d-m-Y' === $date_format) {
						$issued_date = DateTime::createFromFormat('m-Y', $search_value);
						if ($issued_date) {
							$format_issued_date = 'Y-m';
						}
						...
						$issued_date = DateTime::createFromFormat('Y-m', $search_value);
						if ($issued_date) {
							$format_issued_date = 'Y-m';
						}
					} elseif ('Y/m/d' === $date_format) {
					...
					if (!$issued_date) {
						$issued_date = DateTime::createFromFormat('Y', $search_value);
						if ($issued_date) {
							$format_issued_date = 'Y';
						}
					}
				}

				if ($issued_date && $format_issued_date) {
					$conditions[] = '(tc.issued_date LIKE "%' . $issued_date->format($format_issued_date) . '%")';
				}

				$condition = implode(' OR ', $conditions);
				if ($condition) {
					$query_filter .= ' AND (' . $condition . ')';
				}
			}
		}

		$columns = array('tc.certificate_number', 'sr.name', 'sr.admission_number', 'c.label', 'se.label', 'cert.label', 'tc.issued_date', 'sr.is_active', 'tc.ID');
		if (isset($_POST['order']) && isset($columns[$_POST['order']['0']['column']])) {
			$order_by  = sanitize_text_field($columns[$_POST['order']['0']['column']]);
			$order_dir = sanitize_text_field($_POST['order']['0']['dir']);
			...
			$query_filter .= ' ORDER BY tc.created_at DESC';
		}
		...
		$rows_query = WLSM_M_Staff_General::fetch_transfer_certificate_query_count($school_id, $session_id);

		$total_rows_count = absint($wpdb->get_var($rows_query));

		if ($condition) {
			$filter_rows_count = absint($wpdb->get_var($rows_query . ' AND (' . $condition . ')'));
		} else {
			$filter_rows_count = $total_rows_count;
		}

		$transfer_certificates = $wpdb->get_results($query_filter . $limit);
		$certificate_page_url = WLSM_M_Staff_General::get_certificates_page_url();

		$data = array();

		if ($transfer_certificates) {
			foreach ($transfer_certificates as $transfer_certificate) {
				$status_badge = $transfer_certificate->student_is_active
					? '<span class="badge badge-success">' . esc_html__('Active', 'school-management') . '</span>'
					: '<span class="badge badge-secondary">' . esc_html__('Inactive', 'school-management') . '</span>';

				$action = '-';
				$certificate_student_id = isset($transfer_certificate->certificate_student_id) ? absint($transfer_certificate->certificate_student_id) : 0;
				if (!$certificate_student_id) {
					$certificate_student_id = absint($wpdb->get_var($wpdb->prepare(
						"SELECT ID FROM " . WLSM_CERTIFICATE_STUDENT . " WHERE certificate_id = %d AND student_record_id = %d ORDER BY ID DESC LIMIT 1",
				...
				if ($certificate_student_id) {
					...
					$action = '<a href="' . esc_url($view_url) . '" class="btn btn-sm btn-outline-primary" title="' . esc_attr__('View Certificate', 'school-management') . '">' . esc_html__('View', 'school-management') . '</a>';
				}

				$data[] = array(
					esc_html($transfer_certificate->certificate_number),
					esc_html(WLSM_M_Staff_Class::get_name_text($transfer_certificate->student_name)),
					esc_html(WLSM_M_Staff_Class::get_admission_no_text($transfer_certificate->admission_number)),
					esc_html(WLSM_M_Class::get_label_text($transfer_certificate->class_label)),
					esc_html(WLSM_M_Staff_Class::get_section_label_text($transfer_certificate->section_label)),
					esc_html(WLSM_M_Staff_Class::get_certificate_label_text($transfer_certificate->certificate_label)),
					esc_html(WLSM_Config::get_date_text($transfer_certificate->issued_date)),
					$status_badge,
					$action
				);
			}
