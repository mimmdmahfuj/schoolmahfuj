...
if (isset($_GET['id']) && !empty($_GET['id'])) {
	...
	if ($invoice) {
		...
		if ($id) {
			$discount_data    = WLSM_M_Staff_Accountant::get_discount_data($id);
			if ($discount_data) {
				$discount_percent = $discount_data->discount_percent;
				$discount_note    = $discount_data->note;
	...
	<div class="col-md-12">
		...
		<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post" id="wlsm-save-invoice-form">
			...
			<div class="wlsm-form-section">
				...
				<div class="form-row">
					...
					<div class="form-group col-md-6">
						...
						<textarea name="invoice_description" class="form-control" id="wlsm_invoice_description" cols="30" rows="2" placeholder="<?php esc_attr_e('Enter description', 'school-management'); ?>" required><?php echo esc_html(stripslashes($discount_note ?? '')); ?></textarea>
					</div>
				</div>
				...
				<div class="form-row">
					...
					<div class="form-group col-md-3" id="invoice_amount_total">
					...
					<div class="form-group col-md-3">
						...
						<input type="number" step="any" min="0" name="invoice_discount" class="form-control" id="wlsm_invoice_discount" placeholder="<?php esc_attr_e('Enter discount in percent', 'school-management'); ?>" value="<?php echo esc_attr($discount_percent ? WLSM_Config::sanitize_money($discount_percent) : ''); ?>">
					</div>

					<div class="form-group col-md-3">
						<label for="wlsm_invoice_discount_amount" class="wlsm-font-bold">
							<?php esc_html_e('Discount Amount', 'school-management'); ?>:
						</label>
						<input type="number" step="any" min="0" name="invoice_discount_amount" class="form-control" id="wlsm_invoice_discount_amount" placeholder="<?php esc_attr_e('Enter discount in percent', 'school-management'); ?>" value="<?php echo esc_attr($discount_amount ? WLSM_Config::sanitize_money($discount_amount) : ''); ?>">
					</div>
			...
			if (!$invoice) {
				...
				require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/accountant/invoices/discount_log.php';
			}
