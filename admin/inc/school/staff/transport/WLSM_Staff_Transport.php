...
class WLSM_Staff_Transport {
	public static function fetch_vehicles() {
		$current_user = WLSM_M_Role::can( 'view_transport' );
		...
		if ( count( $filter_rows_limit ) ) {
			foreach ( $filter_rows_limit as $row ) {
				...
				// Edit button
				$edit_button = WLSM_M_Role::can('edit_transport') ?
				...
				// Delete button
				$delete_button = WLSM_M_Role::can('delete_transport') ?
	...
	public static function save_vehicle() {
		$current_user = WLSM_M_Role::can( 'add_transport' );
	...
	public static function delete_vehicle() {
		$current_user = WLSM_M_Role::can( 'delete_transport' );
	...
	public static function fetch_hostels() {
		$current_user = WLSM_M_Role::can( 'view_hostel' );
		...
		if ( count( $filter_rows_limit ) ) {
			foreach ( $filter_rows_limit as $row ) {
				...
				// Edit button
				$edit_button = WLSM_M_Role::can('edit_hostel') ?
					'<a class="text-primary" href="' . esc_url( $page_url . "&action=save&id=" . $row->ID ) . '"><span class="dashicons dashicons-edit"></span></a>&nbsp;&nbsp;' :
					'<span class="text-muted"><span class="dashicons dashicons-edit"></span></span>&nbsp;&nbsp;';

				// Delete button
				$delete_button = WLSM_M_Role::can('delete_hostel') ?
