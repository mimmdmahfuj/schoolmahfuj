...
$start_at   = null;
$class_type = 8;
$meeting_type = 2; // Default meeting type
$duration   = 40;
			...
			<div class="form-row mt-3">
				<div class="form-group col-md-12">
					<div class="form-check form-check-inline  btn btn-primary">
						<input <?php echo ( ! $meeting || $meeting->class_type === 'zoom_class' ) ? 'checked' : ''; ?> class="form-check-input" type="radio" name="class_type" id="zoom_class" value="zoom_class">
						<label class="ml-1 form-check-label wlsm-font-bold" for="zoom_class">
							<?php esc_html_e( 'Zoom Live Class', 'school-management' ); ?>
						</label>
					</div>
					<div class="form-check form-check-inline  btn btn-primary">
						<input <?php echo ( $meeting && $meeting->class_type === 'bbb_class' ) ? 'checked' : ''; ?> class="form-check-input" type="radio" name="class_type" id="bbb_class" value="bbb_class">
				...
				<div class="form-row">
					<div class="form-group col-sm-12 col-md-4"  id="class_time">
						...
						<select name="type" class="form-control selectpicker" id="wlsm_type" data-live-search="true" title="<?php esc_attr_e( 'Select Class Type', 'school-management' ); ?>">
							...
							<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $key, $meeting_type, true ); ?>>
					...
					<div class="col-sm-12 col-md-6">
						...
						<div class="form-group">
							...
							if ($staff_id) {
								...
								$is_token_expired = empty($token_expiration) || time() > $token_expiration;
								$url = 'https://zoom.us/oauth/authorize?response_type=code&client_id=' . $settings_zoom_api_key . '&redirect_uri=' . $settings_zoom_api_url;
								?>

								<div class="zoom-auth-status">
									<?php if ($settings_zoom_api_token) : ?>
										<?php if ($is_token_expired) : ?>
											<div class="alert alert-warning mb-2">
												<i class="fas fa-exclamation-triangle"></i>
												<?php esc_html_e('Token has expired. Please generate a new one.', 'school-management'); ?>
											</div>
										<?php else : ?>
											<div class="alert alert-success mb-2">
												<i class="fas fa-check-circle"></i>
												<?php
												$time_left = ceil(($token_expiration - time()) / 60);
												printf(
													esc_html__('Token valid for %d minutes', 'school-management'),
													$time_left
												);
												?>
											</div>
									...
									<a target="blank"
									   ...
									   href="<?php echo esc_url($url); ?>">
										...
										<?php
										echo $settings_zoom_api_token
											? esc_html__('Update Access Token', 'school-management')
											: esc_html__('Generate Access Token', 'school-management');
										?>
									</a>
								</div>
								...
								<span class="text-danger">
									...
									<?php esc_html_e('Login as staff to authorize zoom.', 'school-management'); ?>
								</span>
								<?php
							}
							?>
...
<style>
/* Ensure proper initial state for class type sections */
<?php if ($meeting && $meeting->class_type === 'bbb_class'): ?>
#zoom_optional, #class_approval_type, #recurrence_type { display: none !important; }
<?php else: ?>
#bbb_optional, #class_moderator, #class_viewer { display: none !important; }
<?php endif; ?>
</style>
