...
class WLSM_Staff_Chart {
	...
	public static function fetch_monthly_income_expense() {
		$current_user = WLSM_M_Role::can( array( 'view_income', 'view_expenses' ) );
	...
	public static function fetch_accounting_payment_expense() {
		...
		try {
			...
			if ( ! wp_verify_nonce( $_POST['nonce'], 'accounting-payment-expense' ) ) {
				die();
			}

			// Get date range from POST (default to current month)
			...
			// Fetch paid invoices (income from payments)
			$monthly_payments = $wpdb->get_results(
				$wpdb->prepare( 'SELECT DATE_FORMAT(p.created_at, "%b %Y") as month_year, COALESCE(SUM(p.amount), 0) as sum FROM ' . WLSM_PAYMENTS . ' as p
				JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = p.school_id
				JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = p.student_record_id
				...
				JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
				JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
				WHERE p.school_id = %d AND ss.ID = %d AND CAST(p.created_at AS DATE) BETWEEN %s AND %s GROUP BY DATE_FORMAT(p.created_at, "%Y-%m")', $school_id, $session_id, $start_date, $end_date )
			);

			// Fetch expenses
			$monthly_expenses = $wpdb->get_results(
				$wpdb->prepare( 'SELECT DATE_FORMAT(ep.expense_date, "%b %Y") as month_year, COALESCE(SUM(ep.amount), 0) as sum FROM ' . WLSM_EXPENSES . ' as ep WHERE ep.school_id = %d AND ep.expense_date BETWEEN %s AND %s GROUP BY DATE_FORMAT(ep.expense_date, "%Y-%m")', $school_id, $start_date, $end_date )
			);

			// Combine months from both payments and expenses
			$payment_months = array_map( function( $payment ) {
			...
			$months = array_unique( array_merge( $payment_months, $expense_months ) );
			...
			$payments = array();
			foreach ( $monthly_payments as $value ) {
				$payments[ $value->month_year ] = WLSM_Config::sanitize_money( $value->sum );
			}

			// Organize expense data by month
			$expenses = array();
			foreach ( $monthly_expenses as $value ) {
				$expenses[ $value->month_year ] = WLSM_Config::sanitize_money( $value->sum );
			}
			...
			foreach ( $sorted_months as $month ) {
				array_push(
					...
					array(
						'x' => $month,
						'y' => isset( $payments[ $month ] ) ? WLSM_Config::sanitize_money( $payments[ $month ] ) : WLSM_Config::sanitize_money( 0 )
					)
				);
				...
				array_push(
					...
					array(
						'x' => $month,
						'y' => isset( $expenses[ $month ] ) ? WLSM_Config::sanitize_money( $expenses[ $month ] ) : WLSM_Config::sanitize_money( 0 )
					)
				);
			}
