...
<div class="wlsm-container wlsm" id="wlsm-print-invoice">
	<div class="wlsm-print-invoice-container">
		...
		?>
		<div class="col-md-12">
			<div class="invoice border border-secondary wlsm-print-invoice-container" style="background: no-repeat center/100% url(<?php echo esc_url(wp_get_attachment_url($invoice_card_background)); ?>) !important;">
				...
				<div class="invoice-header">
					...
					<div class="invoice-date">
						...
						<div class="date text-inverse m-t-5"><?php esc_html_e('Date:', 'school-management'); ?> <?php echo esc_html(WLSM_Config::get_date_text($invoice->date_issued)); ?></div>
						<div class="invoice-detail">
							<strong><?php esc_html_e('Due Date:', 'school-management'); ?></strong> <?php echo esc_html(WLSM_Config::get_date_text($invoice->due_date)); ?>
						</div>
						<div class="invoice-detail">
							<strong><?php esc_html_e('Due Date Penalty:', 'school-management'); ?></strong> <?php echo esc_html(WLSM_Config::get_money_text($invoice->due_date_amount, $school_id)); ?>
						</div>
					</div>
				</div>
				<!-- end invoice-header -->
					...
					<?php if ($fee_list) : ?>
						<div class="table-responsive w-100">
							<table class="table table-bordered wlsm-view-fee-structure" style="background-color: white !important;">
								...
								<tfoot>
									<tr>
										<td><strong><?php esc_html_e('Total', 'school-management'); ?></strong></td>
							...
							<tbody>
								<tr>
									...
									if ($invoice->ID) {
										...
										$invoice_discount_amount = $discount_data->amount;
										$discount = rtrim(rtrim(number_format($invoice->discount, 2), '0'), '.');
									}

									echo esc_html($invoice_discount_amount). " ($discount%)"; ?></td>
									...
									<td class="text-center"><?php echo esc_html(WLSM_Config::get_money_text($due + $invoice->due_date_amount, $school_id)); ?></td>
									<td class="text-center">
										<?php
										echo wp_kses(
