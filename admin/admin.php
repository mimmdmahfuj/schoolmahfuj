...
require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/general/WLSM_Setup_Wizard.php';
...
require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/tickets/WLSM_Tickets.php';
...
add_filter('admin_body_class', array('WLSM_Menu', 'add_admin_body_class'));
...
add_action('wp_ajax_wlsm-unassign-classes', array('WLSM_Staff_General', 'unassign_classes'));
...
add_action('wp_ajax_wlsm-save-appearance-settings', array('WLSM_Setting', 'save_appearance_settings'));
...
add_action('wp_ajax_wlsm-generate-dummy-data', array('WLSM_Setting', 'generate_dummy_data'));
add_action('wp_ajax_wlsm-cleanup-demo-data', array('WLSM_Setting', 'cleanup_demo_data'));
...
add_action('wp_ajax_wlsm-print-bulk-staff-id-cards', array('WLSM_Staff_General', 'print_staff_id_cards'));
...
// Staff: Transfer certificates.
add_action('wp_ajax_wlsm-transfer-certificate', array('WLSM_Staff_General', 'transfer_certificate'));
add_action('wp_ajax_wlsm-fetch-student-transfer-details', array('WLSM_Staff_General', 'fetch_student_transfer_details'));
add_action('wp_ajax_wlsm-fetch-transfer-certificates', array('WLSM_Staff_General', 'fetch_transfer_certificates'));
...
add_action('wp_ajax_wlsm-bbb-start-meeting', array('WLSM_Staff_Class', 'bbb_start_meeting'));
add_action('wp_ajax_wlsm-bbb-join-meeting', array('WLSM_Staff_Class', 'bbb_join_meeting'));
...
add_action('wp_ajax_wlsm-save-school-firebase-settings', array('WLSM_Staff_General', 'save_school_firebase_settings'));
...
// Gender management AJAX handlers
add_action('wp_ajax_wlsm-add-gender-option', array('WLSM_Staff_General', 'add_gender_option'));
add_action('wp_ajax_wlsm-edit-gender-option', array('WLSM_Staff_General', 'edit_gender_option'));
add_action('wp_ajax_wlsm-delete-gender-option', array('WLSM_Staff_General', 'delete_gender_option'));
add_action('wp_ajax_wlsm-reorder-gender-options', array('WLSM_Staff_General', 'reorder_gender_options'));
...
add_action('wp_ajax_wlsm-filter-payments-by-method', array('WLSM_Staff_Accountant', 'get_payments_by_method'));
...
add_action('wp_ajax_wlsm-print-expense', array('WLSM_Staff_Accountant', 'print_expense'));
...
add_action('wp_ajax_wlsm-print-income', array('WLSM_Staff_Accountant', 'print_income'));
...
// Staff: Concession Types.
add_action('wp_ajax_wlsm-fetch-concession-types', array('WLSM_Staff_Accountant', 'fetch_concession_types'));
add_action('wp_ajax_wlsm-save-concession', array('WLSM_Staff_Accountant', 'save_concession'));
add_action('wp_ajax_wlsm-delete-concession', array('WLSM_Staff_Accountant', 'delete_concession'));
add_action('wp_ajax_wlsm-get-class-fee-types', array('WLSM_Staff_Accountant', 'get_class_fee_types'));
add_action('wp_ajax_wlsm-get-concession-fee-types', array('WLSM_Staff_Accountant', 'get_concession_fee_types'));

// Staff: Students Concession.
add_action('wp_ajax_wlsm-fetch-students-concession', array('WLSM_Staff_Accountant', 'fetch_students_concession'));
add_action('wp_ajax_wlsm-edit-student-concession', array('WLSM_Staff_Accountant', 'edit_student_concession'));
...
add_action('wp_ajax_wlsm-fetch-accounting-payment-expense', array('WLSM_Staff_Chart', 'fetch_accounting_payment_expense'));

// Staff: Accounting Stats.
add_action('wp_ajax_wlsm-fetch-accounting-stats', array('WLSM_Staff_Accountant', 'fetch_accounting_stats'));
...
add_action('wp_ajax_wlsm-get-class-concessions', array('WLSM_Staff_General', 'get_class_concessions'));
...
// Tickets.
add_action('wp_ajax_wlsm-fetch-tickets', array('WLSM_Tickets', 'fetch_tickets'));
add_action('wp_ajax_wlsm-save-ticket', array('WLSM_Tickets', 'save_ticket'));
add_action('wp_ajax_wlsm-delete-ticket', array('WLSM_Tickets', 'delete_ticket'));
...
add_action('wp_ajax_wlsm-get-class-admit-card-exams', array('WLSM_Staff_General', 'get_class_admit_card_exams'));
add_action('wp_ajax_wlsm-get-class-result-exams', array('WLSM_Staff_General', 'get_class_result_exams'));
...
function wlsm_ensure_admin_capabilities()
{
    $admin_role = get_role('administrator');
    if ($admin_role) {
        $admin_role->add_cap('manage_school_management');
    }
}
add_action('admin_init', 'wlsm_ensure_admin_capabilities');

// Use this function to assign capabilities to specific users
function wlsm_assign_capability_to_users()
{
    // Example: Add specific user IDs here
    $user_ids = []; // Replace with actual user IDs // for demo  ---- add 2
    WLSM_Helper::assign_school_management_capability($user_ids);
    // Remove capability from users
    // WLSM_Helper::remove_school_management_capability(20);
}
add_action('init', 'wlsm_assign_capability_to_users');

// Setup Wizard.
add_action('wp_ajax_wlsm-setup-wizard-subjects', array('WLSM_Setup_Wizard', 'handle_subjects_step'));
add_action('wp_ajax_wlsm-setup-wizard-student-types', array('WLSM_Setup_Wizard', 'handle_student_types_step'));
add_action('wp_ajax_wlsm-setup-wizard-fee-types', array('WLSM_Setup_Wizard', 'handle_fee_types_step'));
add_action('wp_ajax_wlsm-setup-wizard-general-settings', array('WLSM_Setup_Wizard', 'handle_general_settings_step'));
add_action('wp_ajax_wlsm-setup-wizard-registration-settings', array('WLSM_Setup_Wizard', 'handle_registration_settings_step'));
add_action('wp_ajax_wlsm-setup-wizard-complete', array('WLSM_Setup_Wizard', 'handle_complete_step'));

// Public AJAX actions for BigBlueButton
add_action('wp_ajax_wlsm-student-bbb-join', array('WLSM_P_General', 'student_bbb_join_meeting'));
add_action('wp_ajax_nopriv_wlsm-student-bbb-join', array('WLSM_P_General', 'student_bbb_join_meeting'));
