...
require_once WLSM_PLUGIN_DIR_PATH . 'includes/libs/WLSM_Payu.php';
...
?>
	...
	public static function get_student_pending_invoice()
	{
				...
				<div class="wlsm-form-group">
					...
					?>
						...
						<input
							...
							?>
						type="number" step="any" min="0" name="payment_amount" class="wlsm-form-control" id="wlsm_payment_amount" placeholder="<?php esc_attr_e('Enter amount to pay', 'school-management'); ?>" value="<?php echo $due; ?>">
						<?php
							if ( $due_date && $date_now > $due_date ) {
						?>
							<p>
								<span class="wlsm-text-danger"><strong><?php esc_html_e( 'Note: After due date partial payment is disabled.', 'school-management' ); ?></strong></span>
							</p>
						<?php
							}
						?>
		...
		if ($school_payu_enable && WLSM_Payment::currency_supports_payu($currency)) { ?>
			<br>
			<label class="radio-inline wlsm-mr-3">
				<input type="radio" name="payment_method" class="wlsm-mr-2" value="payu" id="wlsm-payment-payu">
				<?php echo esc_html(WLSM_M_Invoice::get_payment_method_text('payu')); ?>
			</label>
		<?php
			$payment_methods_count++;
		}
					...
					if ($school_amberpay_enable && WLSM_Payment::currency_supports_amberpay($currency)) { ?>
						<br>
						<label class="radio-inline wlsm-mr-3">
							<input type="radio" name="payment_method" class="wlsm-mr-2" value="amberpay" id="wlsm-payment-amberpay">
							<?php echo esc_html(WLSM_M_Invoice::get_payment_method_text('amberpay')); ?>
						</label>
					<?php
						$payment_methods_count++;
					}
	...
	public static function get_student_pending_invoice_bulk()
	{
		...
		try {
			...
			foreach ($invoice_ids as $invoice_id ) {
				...
				$school_id  = $invoice->school_id;
				$session_id = isset($invoice->session_id) ? $invoice->session_id : 0;
		...
		if (count($errors) < 1) {
			...
			try {
				...
				if ('razorpay' === $payment_method) {
					...
					$school_payu_url    = $settings_payu['payment_url'];

					if (!$school_payu_enable || !WLSM_Payment::currency_supports_payu($currency)) {
						wp_send_json_error(esc_html__('PayU payment method is currently unavailable.', 'school-management'));
					}

					$amount_formatted = number_format((float) $payment_amount, 2, '.', '');
					$txnid            = substr(hash('sha256', $invoice_id . '|' . microtime(true)), 0, 20);
					$product_info     = substr($invoice_title_number, 0, 100);
					$callback_url     = self::get_payu_callback_url();
					$customer_name    = trim($name) ? wp_strip_all_tags($name) : 'Customer';
					...
					$invoice_ids_value = $invoice_ids;
					if ( is_array( $invoice_ids_value ) ) {
						$invoice_ids_value = implode( ',', array_map( 'absint', $invoice_ids_value ) );
					}
					$invoice_ids_value = $invoice_ids_value ? (string) $invoice_ids_value : '';
					...
					$payu_fields = array(
						...
						'surl'        => $callback_url,
						'furl'        => $callback_url,
						'udf1'        => (string) $invoice_id,
						'udf2'        => $invoice_ids_value,
						'udf3'        => '',
						'udf4'        => '',
						'udf5'        => esc_url_raw( $current_page_url ),
					);

					$computed_hash    = WLSM_Payu::generate_hash( $payu_fields, $school_payu_salt );
					$payload_log_data = $payu_fields;
					$payload_log_data['hash'] = $computed_hash;

					$form_html = WLSM_Payu::build_payment_form( $payu_fields, $school_payu_key, $school_payu_salt, $school_payu_mode );

					$payload_log = wp_json_encode( $payload_log_data );
					if ( false === $payload_log ) {
						$payload_log = '{}';
					}

					error_log( sprintf(
						'[WLSM PayU] Initiating payment | invoice_id=%d | txnid=%s | amount=%s | mode=%s | hash=%s | payload=%s',
							$invoice_id,
							$txnid,
							$amount_formatted,
							$school_payu_mode,
					...
					$pay_with_payu_text = sprintf(
						/* translators: %s: amount to pay */
						__('Pay Amount %s using PayU', 'school-management'),
						esc_html( WLSM_Config::get_money_text( $payment_amount, $school_id ) )
					);

					$message = '<div class="wlsm-payu-message"><p>' . esc_html__( 'Please do not refresh this page while we redirect you to PayU...', 'school-management' ) . '</p></div>';
					$html    = '<div class="wlsm-payu-init">' . $message . $form_html . '</div>';

					$json = wp_json_encode(
						array(
							'payment_method' => sanitize_key( $payment_method ),
							'form_id'        => 'payment_form_submit',
						)
					);
					if ( false === $json ) {
						$json = '{}';
				...
				} elseif ('amberpay' === $payment_method) {
					$settings_amberpay              = WLSM_M_Setting::get_settings_amberpay($school_id);
					$school_amberpay_enable         = $settings_amberpay['enable'];
					...
					if (!$school_amberpay_enable || !WLSM_Payment::currency_supports_amberpay($currency)) {
						wp_send_json_error(esc_html__('Amberpay payment method is currently unavailable.', 'school-management'));
					}

					// Prepare data for the AmberPay form
					$order_id             = $invoice_id;            // Assuming invoice_id is used as order_id
					$payment_amount       = $payment_amount;        // Assuming this is the amount to be paid
					$invoice_title_number = $invoice_title_number;  // Assuming this is the invoice title or number
					$currency             = $currency;              // Assuming this is the currency code
					$current_page_url     = get_permalink();        // Assuming this is the current page URL
					...
					$pay_with_amberpay_text = sprintf(
						/* translators: %s: amount to pay */
						__('Pay Amount %s using AmberPay', 'school-management'),
						esc_html(WLSM_Config::get_money_text($payment_amount, $school_id))
					);

					// create amberpay order
					if ('sandbox' === $school_amberpay_mode) {
						$client_id     = $school_amberpay_client_id;
						$signature_url = $school_amberpay_signature_url;
						$payment_url   = $school_amberpay_payment_url;
						$api_key       = $school_amberpay_api_key;
					} else {
						$client_id     = $school_amberpay_client_id;
						$signature_url = $school_amberpay_signature_url;
						$payment_url   = $school_amberpay_payment_url;
					...
					$signaturePay['ClientId'] = $client_id;
					$signaturePay['TransactionId'] = $transactionId;
					$signaturePay['CurrencyCode'] = $currency;
					$signaturePay['Amount'] = $payment_amount;
					...
					if( $httpcode == 200) {
						...
						$output = json_decode( $output );
						  // Access the Signature property
						$signature = $output->Signature;


					}

					$html = "<button class='wlsm-mt-2 float-md-right button btn btn-success' id='wlsm-stripe-btn'>$pay_with_amberpay_text</button>";
					// var_dump($invoice->student_id); die;

					$html = <<<EOT
							<form action="$school_amberpay_payment_url" method="post">
								<input type="hidden" name="ClientId" value="$school_amberpay_client_id">
								...
								<input type="hidden" name="CustomMessage" value="$invoice_id">
								<input type="hidden" name="CustomerReference" value="$invoice->student_id">
								<input type="hidden" name="PaymentMethod" value="N">
								<input type="hidden" name="BillToEmail" value="$email">
								<input type="hidden" name="BillToTelephone" value="$phone">
								<input type="hidden" name="CustomerInvoice" value="N">
								<input type="hidden" name="BillToAddress" value="$address">
								<input type="hidden" name="BillToCountry" value="N">
								<input type="hidden" name="BillToState" value="N">
								<input type="hidden" name="BillToFirstName" value="$name">
								<input type="hidden" name="BillToCity" value="N">
								<input type="hidden" name="BillToZipPostCode" value="N">
								<input type="hidden" name="CardTokenize" value="N">
								<input type="hidden" name="3DSFlag" value="N">
								<input type="hidden" name="RedirectUrl" value="$current_page_url">
								<button type="submit" class='wlsm-mt-2 float-md-right button btn btn-success' id='wlsm-amberpay-btn'>Pay with AmberPay</button>
							</form>
							EOT;
	...
	public static function process_payu() {
		...
		if ( empty( $_POST['txnid'] ) || empty( $_POST['hash'] ) || empty( $_POST['key'] ) ) {
			return;
		}

		$post_data   = wp_unslash( $_POST );
		$redirect_url = ! empty( $post_data['udf5'] ) ? esc_url_raw( $post_data['udf5'] ) : home_url();
		$invoice_id  = isset( $post_data['udf1'] ) ? absint( $post_data['udf1'] ) : 0;

		if ( ! $invoice_id ) {
			headers_sent() || header( 'Content-Type: text/html; charset=utf-8', true );
			status_header( 200 );
			self::render_payu_response( false, esc_html__( 'Invalid invoice reference.', 'school-management' ), $redirect_url );
			exit;
		}

		$status = isset( $post_data['status'] ) ? sanitize_text_field( $post_data['status'] ) : '';

		$callback_log = wp_json_encode( $post_data );
		if ( false === $callback_log ) {
			$callback_log = '{}';
		}
		error_log( '[WLSM PayU] Callback received: ' . $callback_log );
		...
		try {
			...
			if ( empty( $settings_payu['enable'] ) ) {
				throw new Exception( esc_html__( 'PayU payment method is currently unavailable.', 'school-management' ) );
			}

			$payu_salt = $settings_payu['merchant_salt'];

			if ( ! WLSM_Payu::verify_hash( $post_data, $payu_salt ) ) {
				throw new Exception( $unexpected_error_message );
			}

			if ( 'success' !== strtolower( $status ) ) {
				throw new Exception( esc_html__( 'Payment failed or was cancelled.', 'school-management' ) );
			}

			$payment_amount  = isset( $post_data['amount'] ) ? WLSM_Config::sanitize_money( $post_data['amount'] ) : 0;
			$partial_payment = $invoice->partial_payment;

			$due = $invoice->payable - $invoice->paid + $invoice->due_date_amount;

			if ( ( $payment_amount <= 0 ) || ( $payment_amount > $due ) || ( ! $partial_payment && ( $payment_amount != $due ) ) ) {
				throw new Exception( $unexpected_error_message );
			}

			$transaction_id = ! empty( $post_data['mihpayid'] ) ? sanitize_text_field( $post_data['mihpayid'] ) : sanitize_text_field( $post_data['txnid'] );

			error_log(
				sprintf(
					'[WLSM PayU] Callback verified | invoice_id=%d | txnid=%s | amount=%s | mihpayid=%s',
					$invoice_id,
					isset( $post_data['txnid'] ) ? $post_data['txnid'] : '',
					$payment_amount,
					isset( $post_data['mihpayid'] ) ? $post_data['mihpayid'] : ''
				)
			);
			...
			$inserted = $wpdb->insert( WLSM_PAYMENTS, $payment_data );

			if ( false === $inserted ) {
				throw new Exception( $wpdb->last_error ? $wpdb->last_error : $unexpected_error_message );
			}
			...
			if ( $new_payment_id ) {
				...
				wp_schedule_single_event( time() + 30, 'wlsm_notify_for_online_fee_submission', $event_data );
				wp_schedule_single_event( time() + 30, 'wlsm_notify_for_online_fee_submission_to_parent', $event_data );
			}

			headers_sent() || header( 'Content-Type: text/html; charset=utf-8', true );
			status_header( 200 );
			self::render_payu_response( true, esc_html__( 'Payment made successfully.', 'school-management' ), $redirect_url );
			exit;
		} catch ( Exception $exception ) {
			$wpdb->query( 'ROLLBACK;' );
			...
			error_log( '[WLSM PayU] Callback error: ' . $message );

			headers_sent() || header( 'Content-Type: text/html; charset=utf-8', true );
			status_header( 200 );
			self::render_payu_response( false, $message, $redirect_url );
			exit;
		}
	}

	public static function get_payu_callback_url() {
		return home_url( 'wlsm-payu-callback/' );
	}
	...
	public static function process_amberpay() {
		error_log(print_r($_POST, true));
		if (!(!isset($_POST["Transactionld"]) && !isset($_POST["Transactionld"]))) {
			...
			if (true) {
				self::save_amberapy_payment($data);
			}
		}
	}

	private static function render_payu_response( $success, $message, $redirect_url ) {
		$status_class = $success ? 'wlsm-payu-success' : 'wlsm-payu-error';
		echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' . esc_html__( 'PayU Response', 'school-management' ) . '</title>';
		echo '<style>body{font-family:Arial,sans-serif;background:#f7f7f7;margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100vh;}';
		echo '.wlsm-payu-result{background:#fff;border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,0.1);padding:32px;max-width:480px;width:100%;text-align:center;}';
		echo '.wlsm-payu-success{color:#2d8a34;} .wlsm-payu-error{color:#c62828;}';
		echo '.wlsm-payu-result a{display:inline-block;margin-top:16px;padding:10px 20px;border-radius:4px;text-decoration:none;background:#1e88e5;color:#fff;}';
		echo '.wlsm-payu-result a:hover{background:#1565c0;}';
		echo '</style></head><body>';
		echo '<div class="wlsm-payu-result"><h2 class="' . esc_attr( $status_class ) . '">' . esc_html( $message ) . '</h2>';
		echo '<a href="' . esc_url( $redirect_url ) . '">' . esc_html__( 'Go back', 'school-management' ) . '</a></div>';
		echo '</body></html>';
	}

	private static function save_amberapy_payment($data)
	{
		$unexpected_error_message = esc_html__('An unexpected error occurred!', 'school-management');
