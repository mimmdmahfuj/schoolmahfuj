...
class WLSM_P_Registration {
	public static function submit_registration() {
		...
		try {
			...
			$dob 			 = isset($_POST['dob']) ? DateTime::createFromFormat('Y-m-d', sanitize_text_field($_POST['dob'])) : null;
			...
			$allow_student_login = isset( $_POST['allow_student_login'] ) ? (bool) ( $_POST['allow_student_login'] ) : '';
			...
			$activity_ids 		= (isset($_POST['activities']) && is_array($_POST['activities'])) ? $_POST['activities'] : array();
			$subject_ids_all 	= isset($_POST['subjects_m']) && is_array($_POST['subjects_m']) ? $_POST['subjects_m'] : array();
			$activity_ids_all 	= isset($_POST['activity_m']) && is_array($_POST['activity_m']) ? $_POST['activity_m'] : array();

			// add multiple students.
			$names       = isset($_POST['name_']) && is_array($_POST['name_']) ? $_POST['name_'] : array();
			$genders     = isset($_POST['genders_']) && is_array($_POST['genders_']) ? $_POST['genders_'] : array();
			$dobs        = isset($_POST['dob_']) && is_array($_POST['dob_']) ? $_POST['dob_'] : array();
			$section_ids = isset($_POST['section_id_']) && is_array($_POST['section_id_']) ? $_POST['section_id_'] : array();
			$class_ids  = isset($_POST['class_id_']) && is_array($_POST['class_id_']) ? $_POST['class_id_'] : array();

			$usernames    = isset($_POST['username_']) && is_array($_POST['username_']) ? $_POST['username_'] : array();
			$login_emails = isset($_POST['login_email_']) && is_array($_POST['login_email_']) ? $_POST['login_email_'] : array();
			$passwords    = isset($_POST['password_']) && is_array($_POST['password_']) ? $_POST['password_'] : array();

			$student_types 	= isset($_POST['student_types_']) && is_array($_POST['student_types_']) ? $_POST['student_types_'] : array();

			$pan       		= isset( $_POST['pan'] ) ? sanitize_text_field( $_POST['pan'] ) : '';
			...
			$errors = array();
			foreach($names as $index => $name_) {
				$name_        = isset($names[$index]) ? sanitize_text_field($names[$index]) : '';
				$gender_      = isset($genders[$index]) ? sanitize_text_field($genders[$index]) : '';
				$dob_         = isset($dobs[$index]) ? DateTime::createFromFormat(WLSM_Config::date_format(), sanitize_text_field($dobs[$index])) : null;
				$section_id_  = isset($section_ids[$index]) ? absint($section_ids[$index]) : 0;
				$class_id_    = isset($class_ids[$index]) ? absint($class_ids[$index]) : 0;
				$username_    = isset($usernames[$index]) ? sanitize_text_field($usernames[$index]) : '';
				$login_email_ = isset($login_emails[$index]) ? sanitize_text_field($login_emails[$index]) : '';
				$password_    = isset($passwords[$index]) ? $_POST['password_'] : '';
				$student_type_ = isset($student_types[$index]) ? sanitize_text_field($student_types[$index]) : '';

				$class_school = WLSM_M_Staff_Class::fetch_class( $school_id, $class_id_ );
				if ( ! $class_school ) {
					$errors["class_id_[$index]"] = esc_html__( 'Class not found.', 'school-management' );
				}

				if (empty($name_)) {
					$errors["name_[$index]"] = esc_html__('Please specify student name.', 'school-management');
				}
				// if (empty($dob_)) {
				// 	$errors["dob_[$index]"] = esc_html__('Please specify date of birth.', 'school-management');
				// }
				if(empty($section_id_)) {
					$errors["section_id_[$index]"] = esc_html__('Please select section.', 'school-management');
				}
				if(empty($class_id_)) {
					$errors["class_id_[$index]"] = esc_html__('Please select class.', 'school-management');
				}
				if(empty($username_)) {
					$errors["username_[$index]"] = esc_html__('Please provide username.', 'school-management');
				}
				if(empty($login_email_)) {
					$errors["login_email_[$index]"] = esc_html__('Please provide login email.', 'school-management');
				}
				if(empty($password_)) {
					$errors["password_[$index]"] = esc_html__('Please provide login password.', 'school-management');
				}

				$subject_m_ids = isset($subject_ids_all[$index]) ? $subject_ids_all[$index] : array();
				$activity_m_ids = isset($activity_ids_all[$index]) ? $activity_ids_all[$index] : array();
			}
			if (count($errors) > 0) {
			...
			// Student Login Detail.
			if ( $allow_student_login ) {
				if ( empty( $username ) ) {
					$errors['username'] = esc_html__( 'Please provide username.', 'school-management' );
				}
				if ( empty( $login_email ) ) {
					$errors['email'] = esc_html__( 'Please provide login email.', 'school-management' );
				}
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				$main_student_id = $wpdb->insert_id;

				// multiple students data insert into student records table.
				...
				foreach ($names as $index => $name_) {
					...
					$login_email_  = isset($login_emails[$index]) ? sanitize_text_field($login_emails[$index]) : '';
					$student_type_ = isset($student_types[$index]) ? sanitize_text_field($student_types[$index]) : '';
					$password_     = isset($passwords[$index]) ? $passwords[$index] : '';

					$subject_m_ids = isset($subject_ids_all[$index]) ? $subject_ids_all[$index] : array();
					$activity_m_ids = isset($activity_ids_all[$index]) ? $activity_ids_all[$index] : array();

					if ( ! empty( $dob_ ) ) {
						$dob_ = $dob_->format( 'Y-m-d' );
					} else {
						$dob_ = null;
					}
					...
					// New user.
					$user_data = array(
						'user_email' => $login_email_,
						'user_login' => $username_,
						'user_pass'  => $password_,
					);
					...
					$student_record_data['activities'] = serialize($activity_m_ids);
				...
				if ( count( $fees ) ) {
					foreach ( $fees as $fee ) {
						...
						$student_fee_data = array(
							...
							'label'             => $fee->label,
							'fee_order'         => $fee_order,
							'student_record_id' => $main_student_id,
						);
						...
						if ( $school_registration_create_invoice ) {
							...
							$invoice_data = array(
								'label'                => $student_fee_data['label'],
								'amount'               => $student_fee_data['amount'],
								'invoice_amount_total' => $student_fee_data['amount'],
								'date_issued'          => $student_fee_data['created_at'],
								'due_date'             => $student_fee_data['created_at'],
							);
							...
							$invoice_data['invoice_number']    = $invoice_number;
							$invoice_data['student_record_id'] = $main_student_id;
	...
	public static function submit_staff_registration() {
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				$message = esc_html__( 'Staff User Created Successfully.', 'school-management' );
	...
	public static function submit_school_register() {
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				$message = esc_html__( 'School Created Successfully.', 'school-management' );
